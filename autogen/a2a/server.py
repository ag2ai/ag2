...
class A2aAgentServer:
    ...
    def build_request_handler(
        ...
        *,
        ...
        return DefaultRequestHandler(
            ...
            agent_card=to_core_agent_card(self.card),
            queue_manager=queue_manager,
            push_config_store=push_config_store,
            push_sender=push_sender,
            request_context_builder=request_context_builder,
            extended_agent_card=to_core_agent_card(self.extended_agent_card) if self.extended_agent_card else None,
        )
    ...
    def build_starlette_app(
        ...
        *,
        ...
        ``A2AStarletteApplication`` was removed in a2a-sdk 1.0; the equivalent
        is now assembled by composing ``create_agent_card_routes`` and
        ``create_jsonrpc_routes`` onto a plain ``Starlette`` instance.

        Args:
            rpc_url: Path on which the JSON-RPC endpoint is mounted.
            request_handler: The request handler to use. Defaults to
                ``self.build_request_handler()``.
            context_builder: Optional ``ServerCallContextBuilder``.
            enable_v0_3_compat: Accept v0.3-shaped JSON-RPC payloads in
                addition to 1.0. Defaults to True for backward compatibility.
        ...
        routes = [
            *create_agent_card_routes(agent_card=to_core_agent_card(self.card)),
