...
try:
    ...
    build_jsonrpc_asgi = missing_optional_dependency("build_jsonrpc_asgi", "a2a-sdk[http-server]", e)  # type: ignore[misc]

try:
    from .rest import build_rest_asgi
except ImportError as e:
    build_rest_asgi = missing_optional_dependency("build_rest_asgi", "a2a-sdk[http-server]", e)  # type: ignore[misc]

try:
    from .grpc import build_grpc_server, default_grpc_channel_factory
except ImportError as e:
    build_grpc_server = missing_optional_dependency("build_grpc_server", "a2a-sdk[grpc]", e)  # type: ignore[misc]
    default_grpc_channel_factory = missing_optional_dependency("default_grpc_channel_factory", "a2a-sdk[grpc]", e)  # type: ignore[misc]
