...
The serializer relies on :class:`~autogen.events.BaseEvent.to_dict`
for event payload shapes rather than inventing a parallel event
serializer. Anything outside ``autogen.events`` (``Feedback``,
...
def load_run(path: str | os.PathLike[str]) -> "RunResult":
    ...
    :class:`~autogen.eval.Trace` carries no events, so ``aggregates.tokens`` reads
