...
:class:`~autogen.eval.Scorer` whose grade is a boolean pass/fail derived from that
...
A gated criterion emits ONE :class:`~autogen.eval.Feedback`: ``score`` is the boolean
...
per-task token/time *resource* gate, see :class:`~autogen.eval.BudgetThresholds` — a
different axis.
"""
