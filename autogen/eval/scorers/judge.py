...
criterion and emits exactly one :class:`~autogen.eval.Feedback` key. A
...
scorer only reads the injected :class:`~autogen.eval.Trace` and dicts, the
...
from autogen.events import ToolCallEvent, ToolErrorEvent, ToolResultEvent
    ...
    *,
    ...
    Args:
        ...
        threshold: When set, gate the numeric score into a Pass/Fail — the judge's
            ...
            the judge in :func:`~autogen.eval.scorers.threshold`.
    """
    low, high = scale
    judge = Agent(
        f"judge_{key}",
