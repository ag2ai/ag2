...
A :class:`Variants` is a set of named :class:`~autogen.Agent` instances —
one per variant. :func:`run_variants` runs each variant over the suite via
:func:`~autogen.eval.run_agent` and returns a :class:`VariantRunResult`
...
class Variants:
    """A set of named :class:`~autogen.Agent` variants to compare and rank.
