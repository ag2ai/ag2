...
the same substrate :func:`~autogen.eval.evaluate_traces` grades. Per task it runs the
given :class:`~autogen.Agent` with a
:class:`~autogen.middleware.builtin.telemetry.TelemetryMiddleware` exporting to an
**in-memory** span exporter, then reconstructs the :class:`~autogen.eval.Trace`
...
(:func:`~autogen.eval.runtime.evaluate._grade`) — one path, no drift.
...
from autogen.config import ModelConfig
from autogen.middleware.builtin import TelemetryMiddleware
from autogen.stream import MemoryStream, Stream
...
async def run_agent(
    ...
    *,
    ...
    The given :class:`~autogen.Agent` is run once per task with a
    :class:`~autogen.middleware.builtin.telemetry.TelemetryMiddleware` exporting to an
    in-memory span exporter; the :class:`~autogen.eval.Trace` is then reconstructed
    ...
    :func:`~autogen.eval.evaluate_traces` uses, and — when ``store_dir`` is set — the
    ...
    Args:
        ...
        agent: The :class:`~autogen.Agent` instance to evaluate, reused
        ...
        stream: Optional :class:`~autogen.stream.Stream` to publish eval
        ...
        variant: Tags this run's ``TaskEvaluated`` events with a variant name.
            Set by :func:`~autogen.eval.run_variants` for each variant in a
...
async def run_pairwise(
    ...
    *,
    ...
    Convenience over :func:`~autogen.eval.evaluate_pairwise`: runs each
    ...
    :func:`run_agent` is produce-then-:func:`~autogen.eval.evaluate_traces` for one
