...
Pass a ``stream`` to :func:`~autogen.eval.run_agent` / :func:`~autogen.eval.evaluate_traces`
/ :func:`~autogen.eval.run_variants` and you can observe an evaluation exactly
like you observe an agent: subscribe to these events
(``stream.where(VariantCompleted).subscribe(...)``), or attach the ready-made
:func:`~autogen.eval.console_reporter`.

All are ``__transient__`` — observational only. The durable record of a run is
its persisted :class:`~autogen.eval.RunResult` JSON, not these events.
"""

from typing import TYPE_CHECKING

from autogen.events.base import BaseEvent, Field
