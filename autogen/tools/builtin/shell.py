...
from autogen.annotations import Context, Variable
from autogen.events import BuiltinToolCallEvent, ToolCallEvent
...
class ContainerReferenceEnvironment:
    ...
    via :class:`~autogen.config.openai.containers.ContainerManager`.
    """
...
class ShellToolSchema(ToolSchema):
    ...
    :class:`~autogen.exceptions.UnsupportedToolError` — use
    :class:`~autogen.tools.SandboxShellTool` instead.
    """
...
class ShellTool(Tool):
    ...
    - **Anthropic** — NOT supported. Claude's ``bash`` tool is a client-side
      ...
      :class:`~autogen.exceptions.UnsupportedToolError`. Use
      :class:`~autogen.tools.SandboxShellTool` instead, which runs
