...
class Sandbox(Protocol):
    ...
    - :class:`~autogen.tools.sandbox.adapter.ShellAdapter` —
      one ``run(command)`` per shell command, applied filters, sync facade.
    - :class:`~autogen.tools.sandbox.adapter.CodeAdapter` —
    ...
    async def remove_file(self, path: PurePosixPath) -> None:
        ...
        :class:`~autogen.tools.sandbox.adapter.CodeAdapter` to clean up
