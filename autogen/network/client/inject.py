...
``TaskInject`` lives in framework-core (``autogen.annotations``)
because ``Task`` itself is framework-core; it is re-exported here for
symmetry with the network-only injects so users can ``from
autogen.network.client.inject import TaskInject`` alongside the
others.
"""

from typing import Annotated, Any

from autogen.annotations import Inject
...
# ``autogen.task.TaskInject`` — both resolve to ``ag2.task``.
