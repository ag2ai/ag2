...
"""
    ...
    def _process_prompt(self, messages, sender) -> Tuple[Optional[str], Optional[str]]:
        """
    ...
    def _mtcs_reply(self, prompt: str, ground_truth: str = "") -> str:
        """
        Generate a response using Monte Carlo Tree Search (MCTS) based reasoning.

        This method initiates the MCTS process to explore possible reasoning paths
        based on the provided prompt. It evaluates the paths using a grading system
        and selects the best response based on the specified ground truth.
        ...
        Returns:
            str: The final answer generated by the reasoning process.
        """
        root = ThinkNode(content=prompt, parent=None)
        self._root = root
        answer_nodes = []
