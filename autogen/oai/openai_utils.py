...
def config_list_from_dotenv(
    ...
    for model, config in model_api_key_map.items():
        if isinstance(config, str):
            ...
            logging.warning(f"Unsupported type {type(config)} for model {model} configuration")  # nosemgrep: python.lang.security.audit.logging.logger-credential-leak

        if not config_dict["api_key"] or config_dict["api_key"].strip() == "":
            logging.warning(
                f"API key not found or empty for model {model}. "
