...
from ag2.config.openai.mappers import convert_messages, normalize_usage, response_proto_to_schema, tool_to_api
...
class OrcaRouterClient(LLMClient):
    """OpenAI-compatible chat-completions client for the OrcaRouter gateway.

    OrcaRouter exposes an OpenAI-compatible ``/chat/completions`` surface, so
    this adapter reuses the OpenAI SDK transport and AG2's OpenAI message
    mappers while labelling usage and model responses with the
    ``orcarouter`` provider.
    """
