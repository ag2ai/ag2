...
from ag2.config.openai.openai_client import CreateOptions, OpenAIClient, ReasoningEffort

# OrcaRouter is an OpenAI-compatible meta-router: a single endpoint that fronts
# 150+ upstream models and adds server-side routing (`route`) and per-request
# fallback (`models`). Because the wire protocol is the OpenAI Chat Completions
# API, this provider reuses `OpenAIClient` and only differs by:
#   * a default `base_url` pointing at the OrcaRouter gateway, and
#   * the OrcaRouter-specific `route` / `models` fields, which are forwarded in
#     the request body via `extra_body`.
...
class OrcaRouterConfigOverrides(TypedDict, total=False):
    ...
    api_key: str | None
    base_url: str | None
    # OrcaRouter routing controls (sent via extra_body).
    route: str | None
...
class OrcaRouterConfig(ModelConfig):
    ...
    base_url: str | None = ORCAROUTER_DEFAULT_BASE_URL
    # OrcaRouter-specific routing controls. `route` selects a server-side routing
    # strategy; `models` is an ordered per-request fallback list. Both are folded
    # into the request body (`extra_body`) so they reach the OrcaRouter gateway.
    route: str | None = None
    ...
    def _routing_extra_body(self) -> dict[str, Any] | None:
        """Merge OrcaRouter routing controls into a copy of `extra_body`.

        Explicit `extra_body` entries take precedence over the dedicated
        `route` / `models` fields so callers can always override.
        """
        merged: dict[str, Any] = dict(self.extra_body) if self.extra_body else {}
        if self.route is not None:
            merged.setdefault("route", self.route)
        if self.models is not None:
            merged.setdefault("models", list(self.models))
