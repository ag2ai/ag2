...
# Note: max_consecutive_auto_reply=3 is added to prevent potential infinite loops
...
# max_turns=10 is added to limit the total number of chat turns
chat_result, context_variables, last_agent = initiate_swarm_chat(
    ...
    max_turns=10  # Limit total chat turns to 10
)
```
...
# max_turns=5 is added to limit the total number of chat turns
chat_result, _, _ = initiate_swarm_chat(
    initial_agent=agent_6,
    agents=[agent_6, agent_7],
    user_agent=user_agent,
    messages="start",
    max_turns=5  # Limit total chat turns to 5
)
```
