...
`autogen` (`from autogen import Agent`). [ADR 0010](0010-promote-beta-to-top-level-and-delete-classic.md)
kept that split deliberately (`module-name = "autogen"`) while it focused on
promoting the beta framework and deleting the classic one.
...
- **Hard break, no shim.** There is no `autogen` compatibility package or alias;
  ...
  1.0-scale breaking changes already made in ADR 0010.
- This supersedes the "import name `autogen`" part of ADR 0010.
