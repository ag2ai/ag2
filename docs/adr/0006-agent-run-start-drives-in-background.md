...
Amends [0005](0005-agent-run-turn-is-scoped-to-its-context-manager.md). The core
decision of 0005 stands: entering the `async with agent.run(...)` block opens the
turn *scope* but does **not** advance the turn. This ADR adds an opt-in way to
drive it without awaiting `result()` inline.

## Context

0005 decided `run` opens a scope and the turn advances only while `result()` is
...
the task's exception) is exactly the "task hazard" plumbing 0005 wanted to keep
...
the task is created only when something drives the run — so 0005's
...
- `await result()` creates the task if `start()` did not, then awaits it,
  returning the same reply or re-raising the same failure (0005's idempotency is
...
- **The task is owned by the scope.** `__aexit__` cancels it if still running and
  ...
  turn never outlives the block, which is the property 0005 cared about.

This **supersedes 0005's "no background task" mechanism.** 0005 drove the inline
...
- The background-task hazards 0005 cited (orphaned-task exceptions, a turn
