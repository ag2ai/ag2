...
class TestProgress:
    async def test_chunks_forwarded_as_progress(self) -> None:
        agent = Agent(
            "streamer", config=TestConfig(ModelMessageChunk("Hello, "), ModelMessageChunk("world!"), "Hello, world!")
        )
    ...
    async def test_no_progress_without_token(self) -> None:
        agent = Agent("streamer", config=TestConfig(ModelMessageChunk("a"), ModelMessageChunk("b"), "ab"))
        server = MCPServer(agent)
