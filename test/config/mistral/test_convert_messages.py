...
def test_empty_system_prompt_is_omitted() -> None:
    assert convert_messages(["", ""], [], PydanticSerializer()) == []
...
def test_data_input_is_serialized_as_text() -> None:
    ...
    assert result == [UserMessage(content=PydanticSerializer().encode(data).decode())]
...
def test_compaction_summary_becomes_user_turn() -> None:
    result = convert_messages(
        [], [CompactionSummary(summary="we discussed books", event_count=2)], PydanticSerializer()
    )
...
def test_assistant_text_and_tool_call() -> None:
    ...
    [message] = convert_messages([], [response], PydanticSerializer())
...
def test_constituent_tool_events_do_not_duplicate_the_batch() -> None:
    ...
    result = convert_messages([], [ToolResultsEvent([inner]), inner], PydanticSerializer())
...
class TestImageInput:
    def test_url(self) -> None:
        ...
        [message] = convert_messages([], [request], PydanticSerializer())
...
def test_audio_input_is_unsupported() -> None:
    ...
    with pytest.raises(UnsupportedInputError, match="mistral"):
        convert_messages([], [request], PydanticSerializer())
