...
"""
Integration tests for GeminiStatelessClient with AG2 agents.

These tests verify that GeminiStatelessClient works seamlessly with
AG2's agent system, including AssistantAgent, UserProxyAgent, multi-turn conversations,
and group chat scenarios.

The Gemini client returns rich UnifiedResponse objects with typed content blocks
while maintaining full compatibility with existing agent infrastructure via duck typing.

Run with:
    bash scripts/test-core-llm.sh test/agentchat/test_agent_with_gemini_client.py
"""
...
def _assert_gemini_response_structure(chat_result: Any) -> None:
...
def _create_test_gemini_config(credentials: Credentials) -> dict[str, Any]:
    """Create Gemini client config from credentials."""
    return {
        "config_list": [
            {
                "api_type": "google_stateless",  # Use stateless client
...
def test_gemini_client_simple_chat(credentials_gemini_flash: Credentials) -> None:
    """Test basic chat using Gemini client with real API."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)
    ...
    _assert_gemini_response_structure(chat_result)
...
@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_with_vision_multimodal(credentials_gemini_flash: Credentials) -> None:
    """Test Gemini client with vision/multimodal content using formal image input format."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)
...
@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_multi_turn_conversation(credentials_gemini_flash: Credentials) -> None:
    """Test multi-turn conversation maintains context with Gemini client."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)
...
@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_with_system_message(credentials_gemini_flash: Credentials) -> None:
    """Test Gemini client respects system message configuration."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)
...
@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_cost_tracking(credentials_gemini_flash: Credentials) -> None:
    """Test that Gemini client provides accurate cost tracking."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)
    ...
    # Gemini client should provide accurate cost
    assert "usage_including_cached_inference" in chat_result.cost
    cost_info = chat_result.cost["usage_including_cached_inference"]

    # Cost info can be dict or float depending on client
    if isinstance(cost_info, dict):
        assert len(cost_info) > 0
        # Verify usage fields exist
        first_usage = list(cost_info.values())[0]
        assert "prompt_tokens" in first_usage
        assert "completion_tokens" in first_usage
        assert "total_tokens" in first_usage
        assert "cost" in first_usage
    else:
        # If it's a float, just verify it's positive
        assert cost_info >= 0


@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_group_chat(credentials_gemini_flash: Credentials) -> None:
    """Test Gemini client works in group chat scenarios."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)

    # Create specialized agents with Gemini client
...
@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_run_interface(credentials_gemini_flash: Credentials) -> None:
    """Test Gemini client with ConversableAgent::run() interface."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)
...
@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_content_str_compatibility(credentials_gemini_flash: Credentials) -> None:
    """Test that Gemini client responses work with content_str utility."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)
    ...
    for msg in chat_result.chat_history:
        ...
        try:
            ...
            pytest.fail(f"content_str failed on Gemini client response: {e}")


@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_error_handling_invalid_model(credentials_gemini_flash: Credentials) -> None:
    """Test Gemini client error handling with invalid model."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)
    ...
    with pytest.raises(Exception):  # Gemini will raise error for invalid model
        user_proxy.initiate_chat(assistant, message="Hello", max_turns=1)


@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_sequential_chats(credentials_gemini_flash: Credentials) -> None:
    """Test Gemini client with sequential chats and carryover."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)
...
@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_backwards_compatibility(credentials_gemini_flash: Credentials) -> None:
    """Test Gemini client maintains backwards compatibility with string/dict messages."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)
...
@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_multimodal_with_multiple_images(credentials_gemini_flash: Credentials) -> None:
    """Test Gemini client with multiple images in one request."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)
...
@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_with_group_pattern(credentials_gemini_flash: Credentials) -> None:
    """Test Gemini client with DefaultPattern group orchestration."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)
    ...
    # Verify pattern-based group chat works with Gemini client
    _assert_gemini_response_structure(chat_result)
...
@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_pattern_with_vision(credentials_gemini_flash: Credentials) -> None:
    """Test Gemini client with DefaultPattern and vision/multimodal content."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)
    ...
    # Verify pattern works with multimodal Gemini responses
    _assert_gemini_response_structure(chat_result)
    summary_lower = chat_result.summary.lower()
...
@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_run_group_chat_basic(credentials_gemini_flash: Credentials) -> None:
    """Test Gemini client with run_group_chat interface for basic text messages.
...
@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_run_group_chat_multimodal(credentials_gemini_flash: Credentials) -> None:
    """Test Gemini client with run_group_chat and multimodal content (images)."""
    llm_config = _create_test_gemini_config(credentials_gemini_flash)
...
@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_run_group_chat_content_preservation(credentials_gemini_flash: Credentials) -> None:
...
@pytest.mark.gemini
@run_for_optional_imports(["google.genai"], "gemini")
def test_gemini_client_thinking_mode(credentials_gemini_flash: Credentials) -> None:
    """Test Gemini client with thinking mode (Gemini 2.5+ models).

    Note: This test uses gemini-2.5-flash which supports thinking mode.
    If the model is not available, the test will fail with an error.
    """
    # Create config with thinking mode parameters
    llm_config = {
        "config_list": [
            {
                "api_type": "google_stateless",  # Use stateless client
                "model": "gemini-2.5-flash",  # Gemini 2.5 supports thinking mode
    ...
    try:
        chat_result = user_proxy.initiate_chat(
            assistant, message="What is 15 factorial? Think step by step.", max_turns=1
        )

        _assert_gemini_response_structure(chat_result)

        # Verify thinking tokens tracked if thoughts were generated
        assert "usage_including_cached_inference" in chat_result.cost
        cost_info = chat_result.cost["usage_including_cached_inference"]

        if isinstance(cost_info, dict) and cost_info:
            first_usage = list(cost_info.values())[0]
            # thinking_tokens field should exist (may be 0 if model didn't use thinking)
            assert "thinking_tokens" in first_usage
    except Exception as e:
        # If gemini-2.5-flash is not available or thinking mode not supported, skip gracefully
        if "404" in str(e) or "not found" in str(e).lower():
            pytest.skip("gemini-2.5-flash model not available")
        else:
            raise
