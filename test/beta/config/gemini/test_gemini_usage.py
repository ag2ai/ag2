# Copyright (c) 2023 - 2026, AG2ai, Inc., AG2ai open-source projects maintainers and core contributors
#
# SPDX-License-Identifier: Apache-2.0

"""Tests for Gemini client usage normalization."""

from unittest.mock import MagicMock

from autogen.beta.config.gemini.gemini_client import GeminiClient


def _make_metadata(prompt=100, candidates=20, total=120, cached=None):
    m = MagicMock()
    m.prompt_token_count = prompt
    m.candidates_token_count = candidates
    m.total_token_count = total
    m.cached_content_token_count = cached
    return m


class TestNormalizeUsage:
    def test_normalizes_to_standard_keys(self):
        result = GeminiClient._normalize_usage(_make_metadata())
        assert result["prompt_tokens"] == 100
        assert result["completion_tokens"] == 20
        assert result["total_tokens"] == 120
        # Gemini-native keys preserved
        assert result["prompt_token_count"] == 100
        assert result["candidates_token_count"] == 20

    def test_includes_cache_read_tokens(self):
        result = GeminiClient._normalize_usage(_make_metadata(cached=500))
        assert result["cache_read_input_tokens"] == 500

    def test_no_cache_key_when_none(self):
        result = GeminiClient._normalize_usage(_make_metadata(cached=None))
        assert "cache_read_input_tokens" not in result

    def test_no_cache_key_when_zero(self):
        result = GeminiClient._normalize_usage(_make_metadata(cached=0))
        assert "cache_read_input_tokens" not in result
