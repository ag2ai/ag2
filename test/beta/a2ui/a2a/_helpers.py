...
A client :class:`~autogen.beta.Agent` talks to an :class:`A2AServer` wired with
an :class:`A2UIAgentExecutor` over an in-process httpx ``ASGITransport`` (the
same path the ``test/beta/a2a`` E2E suite uses). The mock LLM doubles here
mirror the sanctioned pattern in ``test/beta/a2a/_helpers.py`` (``RecordingConfig``):
