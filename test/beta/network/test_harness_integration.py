...
# ---------------------------------------------------------------------------
# Hub.unregister cleanup tests
# ---------------------------------------------------------------------------


class TestHubUnregisterCleanup:
    @pytest.mark.asyncio
    async def test_cleans_up_exposed_paths(self) -> None:
        hub = Hub()

        class _FakeAgent:
            name = "agent-1"

        await hub.register(_FakeAgent(), exposed_paths=["/artifacts/"])
        assert "agent-1" in hub._exposed_paths

        await hub.unregister("agent-1")
        assert "agent-1" not in hub._exposed_paths
