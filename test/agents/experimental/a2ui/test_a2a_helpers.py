...
class TestA2UIPartHelpers:
    def test_create_a2ui_part_single_dict(self) -> None:
        ...
        # Single dict is wrapped in {"messages": [dict]}
        assert part.root.data == {"messages": [data]}
        assert part.root.metadata is not None
    ...
    def test_create_a2ui_part_list(self) -> None:
        ...
        part = create_a2ui_part(ops)

        assert isinstance(part.root, DataPart)
        assert part.root.data == {"messages": ops}
    ...
    def test_get_a2ui_datapart_present(self) -> None:
        part = create_a2ui_part({"test": "data"})
        datapart = get_a2ui_datapart(part)
        assert datapart is not None
        assert datapart.data == {"messages": [{"test": "data"}]}  # type: ignore[union-attr]
