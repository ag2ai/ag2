...
## Remyx Research Paper Code Executor

To use [Remyx](https://docs.remyx.ai/resources/explore)'s pre-built Docker environments for research papers, install AG2 with the `remyx` option:
```bash
pip install "ag2[remyx]"
```

Remyx provides instant access to reproducible environments for 1000+ ArXiv papers with all dependencies pre-installed and tested. Get your API key from remyx.ai.
...
executor.explore(goal="Run the quickstart example with my model", interactive=False)
```
