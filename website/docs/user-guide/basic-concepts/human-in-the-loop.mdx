...
Think of Human in the Loop like a financial advisor working with you on investment decisions:

- The advisor does the research, runs the numbers, and makes recommendations quickly
- But for major decisions, they consult with you before executing the trade
- You provide the final approval, especially for high-risk or unusual investments
- The advisor handles all the details once you've made the call

This partnership leverages both the advisor's analytical capabilities and your judgment, particularly for decisions with significant consequences.
...
Creating a Human in the Loop workflow in AG2 is straightforward using ConversableAgent with the `human_input_mode` parameter:
...
- `ALWAYS`: The agent always asks for human input before responding
...
In our financial compliance system, the workflow follows this pattern:

```mermaid
flowchart TD
    Start([Start]) --> Finance[Finance_bot]
    Finance -- "1.Please approve these" --> Human[Human Agent]
    Human -- "2.Approved" --> Finance
    Finance -- "3.All approved" --> Human
    Human -- "Exit" --> End([End])
```
...
For suspicious transactions, you'll be prompted to provide input - either type **approve**, **deny**, or provide reasoning.
