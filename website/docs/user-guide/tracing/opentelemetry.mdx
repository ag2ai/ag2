...
|---|---|---|
| Conversations | `conversation {agent.name}` | `initiate_chat` / `a_initiate_chat` / `resume` |
...
See the [Advanced: Message Capture](#advanced-message-capture) section for details.
...
|---|---|---|
| `conversation` | `conversation` | `initiate_chat`, `a_initiate_chat`, `resume`, `run_chat`, `a_run_chat` |
...
AG2 tracing captures messages at different levels depending on the span type:

| Span type | `gen_ai.input.messages` / `gen_ai.output.messages` | Controllable? |
|---|---|---|
| `conversation` (initiate_chat) | **Always captured** | No |
| `agent` (generate_reply) | **Always captured** | No |
| `llm` (LLM API call) | **Off by default** | Yes, via `capture_messages` |

Conversation and agent spans always include input and output messages because these attributes are essential for understanding the flow of a multi-agent conversation. This means that **message content will appear in your tracing backend even with default settings**.

!!! warning
    If your agents process sensitive data (personal information, credentials, proprietary content), be aware that conversation and agent spans will contain that data. Ensure your tracing backend has appropriate access controls and retention policies.
