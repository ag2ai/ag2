...
| `gen_ai.input.messages` | string (JSON) | Input messages in OTEL format | `conversation`, `agent` (opt-in; see [Content Capture](#content-capture)), `llm` (opt-in; see [Enabling LLM Message Capture](#enabling-llm-message-capture)) |
| `gen_ai.output.messages` | string (JSON) | Output messages in OTEL format | `conversation`, `agent` (opt-in; see [Content Capture](#content-capture)), `llm` (opt-in; see [Enabling LLM Message Capture](#enabling-llm-message-capture)) |
...
| `gen_ai.tool.call.arguments` | string (JSON) | Tool call arguments | `tool` (opt-in; see [Content Capture](#content-capture)) |
| `gen_ai.tool.call.result` | string | Tool call result | `tool` (opt-in; see [Content Capture](#content-capture)) |
...
| `ag2.human_input.prompt` | string | Prompt shown to the human | `human_input` (opt-in; see [Content Capture](#content-capture)) |
| `ag2.human_input.response` | string | Human's response | `human_input` (opt-in; see [Content Capture](#content-capture)) |
| `ag2.code_execution.exit_code` | int | Exit code from code execution | `code_execution` |
| `ag2.code_execution.output` | string | Output from code execution (truncated to 4096 chars) | `code_execution` (opt-in; see [Content Capture](#content-capture)) |
...
| `ag2.chats.summaries` | string (JSON) | List of per-chat summary strings | `multi_conversation` (opt-in; see [Content Capture](#content-capture)) |
| `ag2.chats.ids` | string (JSON) | List of chat UUIDs | `multi_conversation` |
| `ag2.chats.prerequisites` | string (JSON) | Dependency map between chats (async `initiate_chats` only) | `multi_conversation` |
...
| Span type | Data captured | Default / control |
|---|---|---|
| `conversation` (`run` / `initiate_chat`) | Input/output messages | Off by default; set `AG2_OTEL_RECORD_CONTENT=1` to enable |
| `agent` (`generate_reply`) | Input/output messages | Off by default; set `AG2_OTEL_RECORD_CONTENT=1` to enable |
| `tool` (`execute_function`) | Tool arguments and results | Off by default; set `AG2_OTEL_RECORD_CONTENT=1` to enable |
| `human_input` (`get_human_input`) | Prompt shown and human response | Off by default; set `AG2_OTEL_RECORD_CONTENT=1` to enable |
| `multi_conversation` (`initiate_chats`) | Chat summaries | Off by default; set `AG2_OTEL_RECORD_CONTENT=1` to enable |
| `code_execution` | Script output | Off by default; set `AG2_OTEL_RECORD_CONTENT=1` to enable |
| `llm` (LLM API call) | Request/response messages | Off by default; pass `capture_messages=True` to `instrument_llm_wrapper` |

Content-bearing attributes on `conversation`, `multi_conversation`, `agent`, `tool`, `human_input`, and `code_execution` spans are disabled by default. Set the `AG2_OTEL_RECORD_CONTENT` environment variable to enable them (see [Content Capture](#content-capture) below). LLM (`chat`) spans are controlled separately via `capture_messages=True` on `instrument_llm_wrapper` (see [Enabling LLM Message Capture](#enabling-llm-message-capture)).

!!! warning
    Enabling `AG2_OTEL_RECORD_CONTENT` or `capture_messages=True` sends conversation
    messages, tool arguments and results, human input prompts and responses, and
    code execution output to your tracing backend. Enable content capture only in
    development or trusted environments, and ensure your backend has appropriate
    access controls and data retention policies.

### Content Capture

By default, AG2 tracing does **not** capture message content, tool arguments, human input, or code execution output on any span. This follows the <a href="https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-spans/" className="external-link" target="_blank" rel="noopener noreferrer">OpenTelemetry GenAI Semantic Conventions</a> recommendation that instrumentation SHOULD NOT record prompt and response content by default, as it may contain PII, API keys, or proprietary data.

To enable content capture across all non-LLM span types, set the `AG2_OTEL_RECORD_CONTENT` environment variable before starting your application:

```bash
# Enable content capture (development / debugging only)
AG2_OTEL_RECORD_CONTENT=1 python your_script.py
```

Accepted values: `1`, `true`, `yes` (case-insensitive). Any other value (including unset, `0`, `false`, `no`) leaves content capture disabled. The flag is read once at module import time, so it must be set before the Python process starts -- changing `os.environ` at runtime has no effect.

When enabled, the following additional attributes appear on their respective spans:

| Span type | Attributes added |
|---|---|
| `conversation`, `agent` | `gen_ai.input.messages`, `gen_ai.output.messages` |
| `multi_conversation` | `ag2.chats.summaries` |
| `tool` | `gen_ai.tool.call.arguments`, `gen_ai.tool.call.result` |
| `human_input` | `ag2.human_input.prompt`, `ag2.human_input.response` |
| `code_execution` | `ag2.code_execution.output` |

For LLM (`chat`) spans, content capture is controlled separately via the `capture_messages` argument to `instrument_llm_wrapper` (see [Enabling LLM Message Capture](#enabling-llm-message-capture) below).
