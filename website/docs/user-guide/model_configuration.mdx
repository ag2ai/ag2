...
| a number schema bounded to `0..1`, e.g. `#!python ResponseSchema.from_schema({"type": "number", "minimum": 0, "maximum": 1}, name="churn", description="How likely is churn?")` | a yes/no question | the raw yes-probability as a `float` |
...
The SDK answer, minus its `type`, is kept on `reply.response.metadata`: `confidence` and `probabilities` for every primitive, plus the raw `choice`, `noul` (the yes-probability) or `score` (the expected value on the rubric, before snapping) with its `legend`.
...
See `examples/typesafe/` for a ticket router, a yes/no gate in front of a generative agent, and a severity rubric.
