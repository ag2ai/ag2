...
Runnable scripts under `examples/typesafe/`:

- `answer_kinds.py` — all three answer kinds on one message, printing what each puts on `reply.response.metadata`
- `docstrings_matter.py` — the same `Enum` with and without member docstrings, side by side
- `ticket_router.py` — an `Enum` choice with confidence
- `yes_no_guard.py` — a `bool` gate with `criteria` and `boolean_threshold` in front of a generative agent
- `severity_score.py` — an `IntEnum` rubric
