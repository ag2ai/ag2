...
**A capability is advertised only when you enabled it**, so a conforming server never asks for what you would refuse. Nothing to keep in step by hand: the client derives what it declares from which answering callbacks are supplied, and not enabling one *is* how it goes unadvertised. A server that asks anyway is refused, and the two refusals do not look alike: a question is declined on the wire, so the server can degrade deliberately, while sampling and roots have no decline arm — the error returned for one of those ends the request loop, so the *tool call* fails with that message rather than the server hearing an answer.
