...
!!! note "Going the other way"
    Both options above *consume* an MCP server. To expose an AG2 agent **as** one, see [Serving Resources over MCP](/docs/user-guide/tools/mcp_serving) for its resources and change notifications, and [MCP-UI](/docs/user-guide/tools/mcp_ui) for renderable UI results.
