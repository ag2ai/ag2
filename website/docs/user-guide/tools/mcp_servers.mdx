...
When servers expose the same tool name, give each toolkit a local prefix. The
prefix is visible to the model, while calls sent to the MCP server keep the
original name:
...
agent_with_mcp = Agent(
    name="Mixed bot",
    tools=[
        MCPToolkit("https://github.example.com/mcp", tool_name_prefix="github_"),
        MCPToolkit("https://docs.example.com/mcp", tool_name_prefix="docs_"),
    ],
)
```
