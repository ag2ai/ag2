...
```bash
pip install "tenki-sandbox>=0.4.0,<1"
```
...
`TenkiEnvironment` reads `TENKI_API_KEY` and `TENKI_API_URL`. It auto-selects a sole visible project; pass `project_id` when the key has access to more than one. See the [Tenki Sandbox extension guide](/docs/user-guide/extensions/tenki) for lifecycle and resource configuration.
  </Tab>
...
async with TenkiEnvironment() as env:
    agent = Agent(
        "analyst",
        config=AnthropicConfig(model="claude-sonnet-4-6"),
        tools=[SandboxCodeTool(env)],
...
The same pattern works with `DaytonaEnvironment`, `DockerEnvironment` (container stopped + removed), and any other backend that implements `__aenter__` / `__aexit__`.

### Credentials and runtime config

`TenkiEnvironment` accepts [`Variable`](/docs/user-guide/context/variables) markers for `api_key`, `api_url`, `project_id`, `image`, and `env_vars`. `DaytonaEnvironment` accepts them for `api_key`, `api_url`, `target`, `image`, `snapshot`, and `env_vars`. `DockerEnvironment` accepts them for `image`, `env_vars`, and `network_mode`. Variables resolve from `context.variables` on the first `run_code` call — useful for multi-tenant setups:
...
env = TenkiEnvironment(
    api_key=Variable("tenki_key"),  # resolved from ctx.variables["tenki_key"]
