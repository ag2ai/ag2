...
| :--- | :--- | :--- |
| **Where it runs** | Provider's sandbox | A `CodeEnvironment` you supply (Tenki, Daytona, Docker, custom) |
| **Setup** | Add the tool, done | Choose / configure a backend |
| **Cost** | Bundled in provider tokens | Your sandbox bill (Tenki/Daytona) or free (local Docker) |
...
Some providers expose a server-side Python sandbox the model can drive directly. AG2 surfaces this through [`CodeExecutionTool`](/docs/user-guide/tools/builtin_tools#code-execution) — a declaration-only tool that maps to each provider's native capability:
...
See the [Built-in Tools page](/docs/user-guide/tools/builtin_tools#code-execution) for the provider support matrix and version pinning.
...
`SandboxCodeTool` exposes a `run_code(code, language)` function the agent can call. Because it's a regular function tool, it works on **any** model provider. Where the code actually runs is decided by the [`CodeEnvironment`](https://github.com/ag2ai/ag2/blob/main/ag2/tools/code/environment/base.py){.external-link target="_blank"} you hand it.

!!! note "`environment` is required"
    `SandboxCodeTool` has no default backend. Pass `TenkiEnvironment().code_environment()`, `DaytonaEnvironment`, `DockerEnvironment`, or your own `CodeEnvironment` implementation. Tenki needs the `code_environment()` call because its guest image ships `python3` rather than `python`.
...
Three environments are available: **Tenki** and **Daytona** (hosted), plus **Docker** (local container). You can also implement your own — see the **Custom** tab below.
...
async with env:
    agent = Agent(
        "analyst",
        config=AnthropicConfig(model="claude-sonnet-4-6"),
        tools=[SandboxCodeTool(env.code_environment())],
    )
    reply = await agent.ask("Compute the 50th Fibonacci number in Python.")
    print(await reply.content())
```

`TenkiEnvironment` reads `TENKI_API_KEY` and `TENKI_API_URL`. It auto-selects a sole visible workspace; pass `workspace_id` when the key has access to more than one. `code_environment()` supplies a `CodeAdapter` matching the guest image, whose Python interpreter is `python3`. See the [Tenki Sandbox extension guide](/docs/user-guide/extensions/tenki) for lifecycle and resource configuration.
  </Tab>
...
```bash
pip install "ag2[anthropic]" "daytona>=0.171.0,<1"
```
...
```bash
pip install "ag2[anthropic]" "docker>=6.0.0,<8"
```
...
The `context` argument is the active `ConversationContext`, forwarded so backends can resolve [`Variable`](/docs/user-guide/context/variables) markers from `context.variables` (e.g. per-tenant credentials). Backends with no runtime-configurable parameters can ignore it.
  </Tab>
</Tabs>
...
The same pattern works with `TenkiEnvironment`, `DockerEnvironment` (container stopped + removed), and any other backend that implements `__aenter__` / `__aexit__`.

### Credentials and runtime config

`DaytonaEnvironment` accepts [`Variable`](/docs/user-guide/context/variables) markers for `api_key`, `api_url`, `target`, `image`, `snapshot`, and `env_vars`. `TenkiEnvironment` accepts them for `api_key`, `api_url`, `workspace_id`, `image`, and `env_vars`. `DockerEnvironment` accepts them for `image`, `env_vars`, and `network_mode`. Variables resolve from `context.variables` on the first `run_code` call — useful for multi-tenant setups:
