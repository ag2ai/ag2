...
from autogen.tools import CodeExecutionTool
...
`SandboxCodeTool` exposes a `run_code(code, language)` function the agent can call. Because it's a regular function tool, it works on **any** model provider. Where the code actually runs is decided by the [`CodeEnvironment`](https://github.com/ag2ai/ag2/blob/main/autogen/tools/code/environment/base.py){.external-link target="_blank"} you hand it.
...
The `context` argument is the active `ConversationContext`, forwarded so backends can resolve [`Variable`](/docs/user-guide/tools/tools#variables) markers from `context.variables` (e.g. per-tenant credentials). Backends with no runtime-configurable parameters can ignore it.
  </Tab>
</Tabs>
