...
To run async hooks around **one** tool (e.g. argument normalization, result redaction, bundled auditing), pass `middleware=[...]` with `ToolMiddleware` callables. See the dedicated [Tool middleware](/docs/user-guide/tools/tool_middleware) page.
...
```python linenums="1"
from autogen import Context, tool
...
The framework provides typed input classes for text, structured data, images, audio, video, documents, and raw binary payloads. See [Multimodal Inputs](/docs/user-guide/multimodal/inputs) for the full reference including provider support and factory variants.

Return any input type directly from a tool function just like you would a string:

```python linenums="1"
from autogen import DataInput, TextInput, tool
...
```python linenums="1"
from autogen import ImageInput, ToolResult, tool
...
```python linenums="1"
from autogen import BinaryInput, ToolResult, tool
...
```python linenums="1"
from autogen import Agent, DataInput, TextInput, ToolResult, tool
