...
!!! danger "No transport security settings means DNS rebinding protection is **off**"
    `security_settings=None` — the default — is not a neutral value: the SDK reads an absent value as an instruction to disable the protection, for backwards compatibility. A served agent therefore has none until you name the hosts and origins it answers on.

    A *default-constructed* `TransportSecuritySettings()` is not the safe middle ground either. It enables the protection with **empty** allow-lists, so every request is refused. Behind a reverse proxy, list the public names your deployment answers on (`"agent.example.com"`, `"agent.example.com:*"` to allow any port) — AG2 does not guess them from `security.resource_url`, because that URL cannot see your topology.
...
What counts is whether you asked, not what the number is — supplying `1800.0`, the default value, is refused too. Two things are not asking, and both serve stateless happily:

- **leaving the field alone** — a default you never chose is not a choice;
