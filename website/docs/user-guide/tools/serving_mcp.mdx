...
# ClientModel tunes the channel, the same shape as sessions=:
app = MCPServer(agent, client_model=ClientModel(max_tokens=512))
```

It is off unless you ask for it, and a server that has not enabled it never sends a sampling request and never needs the capability of its clients. `max_tokens` is the only generation parameter that travels, because it is the only one the protocol makes mandatory — the rest belong to a model configuration, and a borrowed model has none here.
