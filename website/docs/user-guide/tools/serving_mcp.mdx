...
`ResourceTemplate` readers receive variable values percent-decoded once as UTF-8. For example, reading `files:///hello%20world.txt` through `files:///{name}` passes `{"name": "hello world.txt"}` to the reader. Use these values directly: `%2520` becomes the literal `%20`, and a literal `+` stays `+`. Decoding happens after template matching; the returned resource URI keeps the original encoding.
