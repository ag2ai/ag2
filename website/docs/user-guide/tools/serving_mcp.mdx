...
!!! warning "Stateless serving and pausing pull against each other"
    A server that can pause a run cannot be freely stateless. Both of the above fail *intermittently* under a load balancer, and the symptom is an occasional "Invalid or expired requestState" with nothing pointing at the cause. See `docs/adr/0018-a-paused-mcp-run-is-held-in-process.md`.
