...
A client that advertised no sampling capability is never asked. What happens then is read off the agent, not off a second switch: one with a `config` of its own falls back to it, and one without fails the call with `MCPSamplingUnavailableError` — a JSON-RPC error `-32021` whose `data.requiredCapabilities` names `sampling`, not a tool result, since no rewording by the model supplies a capability the client lacks. So `client_model=` decides only whether the caller's budget may be spent at all — a deployment holding credentials it would rather not spend keeps them as the fallback simply by configuring them.
...
`transport=` groups every streamable-HTTP setting that has no other route onto the server. The values below are **AG2's own defaults**, passed through explicitly — not whatever the resolved `mcp` release happens to use, so a deployment's behaviour does not move under you when a patch release retunes one.

```python linenums="1"
from ag2 import Agent
from ag2.config import AnthropicConfig
from ag2.mcp import MCPServer, TransportConfig, TransportSecuritySettings
...
Omitting `transport=` is exactly passing `TransportConfig()`.

#### DNS rebinding protection is derived, not guessed

Leave `security_settings` alone and AG2 puts the protection up itself — provided you configured `security=`. `security.resource_url` is the endpoint you declared this server answers on, so it is not a guess about your topology; `allowed_hosts` and `allowed_origins` come straight from it. A URL that names no port allows any, so a proxy forwarding `Host: agent.example.com:8443` still reaches you.

!!! danger "Two cases still serve with no protection at all"
    - **No authorization requirement.** Without `security=` there is no declared URL to derive from, so the protection stays off — the meaning an absent value has always had to the SDK.
    - **An explicit refusal.** `TransportSecuritySettings(enable_dns_rebinding_protection=False)` turns it off deliberately, and wins over the derived settings.

    A *default-constructed* `TransportSecuritySettings()` is not a safe middle ground either: it enables the protection with **empty** allow-lists, so every request is refused. Pass your own lists when your deployment answers on names `resource_url` does not cover (`"agent.example.com:*"` allows any port).

#### Two different session bounds in one call
...
A stateless transport issues no `mcp-session-id`, so there are no MCP sessions to reap. Asking for reaping anyway is refused at construction rather than silently ignored:

```python
MCPServer(agent, stateless=True, transport=TransportConfig(mcp_session_idle_timeout=60.0))
# ValueError: stateless=True issues no mcp-session-id, so transport.mcp_session_idle_timeout (60.0) ...
```

What counts is the value, not how it arrived. Two things are not asking, and both serve stateless happily:

- **the default `1800.0`** — whether you left the field alone or typed the number, it says what the default already says;
- **`mcp_session_idle_timeout=None`** — that asks the transport *not* to reap, which a stateless transport cannot contradict.
...
#### Refusing a token minted for another service

`validate_token_resource=True` turns on the RFC 8707 check: the server then accepts only a token whose resource indicator (`AccessToken.resource`) names `resource_url`, so a token issued for a different service cannot be replayed against your agent.

```python linenums="1"
security = require(
    oauth2_scheme(url="https://auth.example.com"),
    resource_url="https://agent.example.com/mcp",
    verifier=my_token_verifier,
    validate_token_resource=True,
)
```

It ships **off**. The resource field is optional on the token your own verifier builds, and an absent value fails the check — so turning it on for everyone would answer `401` to every deployment whose verifier does not populate it. Enable it when your authorization server binds tokens to the `resource` the client asked for; leave it off when your verifier checks the audience itself.

!!! note "The SDK makes this its default in 3.0"
    `mcp` defaults the same check off through 2.x and on in 3.0. AG2 flips its default when that release lands, so enable it deliberately now rather than meeting it as a surprise then.
...
| `transport` | `None` | a `TransportConfig` of streamable-HTTP settings; omitting it is a default-constructed one |
...
| field | default | what it is for |
|---|---|---|
| `mcp_session_idle_timeout` | `1800.0` | seconds an MCP session may idle before the transport closes it; its next request gets `404`. `None` never reaps |
| `max_mcp_sessions` | `10_000` | concurrent MCP sessions; one beyond the cap is answered `503`. `None` removes the cap |
| `max_request_body_size` | `4 MiB` | a larger body is refused `413`, before it is parsed and before a session is created |
| `security_settings` | `None` | DNS rebinding protection. **`None` disables it**; a default-constructed `TransportSecuritySettings()` refuses everything |
| `sse_retry_interval` | `None` | milliseconds to suggest a client waits before reconnecting a dropped SSE stream. Carried on the priming event, so it does nothing without an `event_store` and a client on 2025-11-25 or later |
| `event_store` | `None` | makes connections resumable: a client reconnecting with `Last-Event-ID` receives what it missed. Bring your own; what is proven is one end-to-end test over a real socket (`test/mcp/test_resumability.py`) |

Non-positive bounds are refused when the `TransportConfig` is constructed.
...
`MCPServer`, `SessionConfig`, `TransportConfig`, `MCPFunctionTool`, `mcp_tool`, `MCPApp`, `AppSandbox`, `Resource`, `ResourceTemplate`, `Prompt`, `PromptArgument`, `PromptMessage`, `AskContext`, `ContextProvider`, `ExtensionMap`, `client_extension`, `build_ask_tool`, and the five curated MCP SDK names `Resolve`, `Elicit`, `Sample`, `ListRoots`, `RequestStateSecurity`.
...
`MCPResourceNotFoundError`, `MCPPromptNotFoundError` and `MCPSamplingUnavailableError` are also the SDK's `MCPError`, so each reaches the caller as a JSON-RPC protocol error rather than a tool result: `-32602` (Invalid params) for an unknown resource URI — with the URI in `data.uri` — or prompt name, and `-32021` for a missing client capability. A `tools/call` naming a tool the server does not have, and a `prompts/get` omitting an argument its prompt declares `required`, are answered `-32602` the same way, before any handler or renderer runs.
...
- **MCP Apps** — pass `apps=[MCPApp(...)]` to serve an interactive document and its bound tools together; see [MCP Apps](/docs/user-guide/tools/mcp_apps).
...
!!! note "`ResourceTemplate` readers receive decoded variables"

    Variable values are percent-decoded once as UTF-8, so reading `files:///hello%20world.txt` through `files:///{name}` passes `{"name": "hello world.txt"}` to the reader. Use these values directly: `%2520` becomes the literal `%20`, and a literal `+` stays `+`. Decoding happens after template matching, and the returned resource URI keeps the original encoding.

    Decoding never widens what a form matches. A `{name}` value is still one path segment after decoding, so `files:///a%2Fb` matches no `files:///{name}` template — serving it would hand the reader the separator the form promised to keep out. A `{+name}` value may span `/` and may contain `..`, so a reader behind the reserved form owns its own containment checks. A URI whose escapes are not valid UTF-8 matches no template rather than decoding to replacement characters.
