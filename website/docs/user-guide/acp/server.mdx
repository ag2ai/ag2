...
| `#!python session/load` | Reattaches to a conversation by id, replays its history, then returns — see [Loading a session](#loading-a-session) |
...
A client that still holds a session id can come back to it with `#!python session/load` —
from a new connection, or a new process, as long as the session's history is still in
storage. The agent replays the whole conversation as `#!python session/update`
notifications and then answers. The response is the line between history and whatever
comes next: nothing is sent after it, and the next `#!python session/prompt` continues the
conversation where it left off.

How far that reaches is decided by two settings on `#!python SessionConfig`:

- **`#!python storage`** — where history lives. The default is in-memory, so it lasts as
  long as the process does. Pass a durable `#!python Storage` — the
  `#!python RedisStorage` that ships with `ag2`, or your own — for history that outlives it.
- **`#!python retain_history`** — whether a session's history is kept when the session is
  *let go*: evicted to make room, expired by `#!python ttl`, or dropped because its
  connection closed. Off by default, since retaining in the default in-memory storage only
  grows the process. Turn it on together with a durable storage. An explicit
  `#!python SessionStore.close` still deletes; that is the one act that means the
  conversation is over — and, since ACP's `#!python session/delete` is not wired, the
  only one. Nothing else in `ag2` ever removes a retained history: `#!python max_sessions`
  and `#!python ttl` bound the *live* registry, not the storage behind it. With a durable
  storage, retention is the host's to manage — expire or cap it in the storage itself, or
  close sessions whose conversations are over.
...
acp_agent = ACPAgent(
    agent,
    sessions=SessionConfig(storage=RedisStorage("redis://localhost:6379"), retain_history=True),
)
```

The id is enough on its own: it is the key the history is stored under, so nothing else has
to be persisted for a later process to find the conversation. A host keeping its own record
per conversation can tie the two together in `#!python on_session`, which is called with the
session and `#!python "created"` after `#!python session/new` and `#!python "loaded"` after
`#!python session/load`. A hook that raises refuses the request: nothing is registered, the
conversation in storage is untouched, and a `#!python session/prompt` on that id is answered
as unknown.

What comes back is the history, and only the history:

- User turns, replies, tool calls and their results, in order — one
  `#!python agent_message_chunk` per reply, since the chunks a live turn streams are never
  stored.
- **Not** the session's context variables: a session loaded in a new process starts them
  from the agent's defaults again. Not the `pending_messages` inbox, and not a compaction
  summary. A session loaded on the connection it already lives on keeps all of those — only
  the request context (`#!python cwd` and friends) is refreshed.
- A turn that was cut short — the process died with a tool still running — is repaired
  first, so the next prompt sends a valid conversation to the provider.

A session with a turn in progress is refused rather than replayed underneath it. A session
that was opened and never prompted has no history, so from another process it is
indistinguishable from an id that was never issued — and is answered the same way.

!!! warning "A session id is a credential"
    Any connection that has passed `#!python initialize` (and `#!python authenticate`, if
    a provider is configured) may load any id it holds. The id is the whole credential: a
    `#!python uuid4`, unguessable, and bound to nothing else — ACP itself defines no link
    between a session and an authenticated party. A host that needs one enforces it at its
    transport, or refuses the load in `#!python on_session`.

    Since the id is also the storage key, it now appears wherever stream ids are logged.

!!! warning "One live process per session id"
    Two processes loading the same id would interleave their writes to one history, and
    the repair a load performs — a read of the whole transcript followed by a rewrite —
    could discard what the other wrote in between. The storage holds no lease and `ag2`
    does not add one. Within a process this is caught: a second load of an id whose turn
    is running elsewhere is refused. Across processes it is the client's contract to
    keep, and a client that spawns one agent process per turn keeps it by construction.

Over stdio and WebSocket this all just works. The ACP SDK's HTTP/SSE server, as of 0.12,
does not register a *loaded* session's stream the way it does a new one, so a
`#!python session/prompt` on a loaded session over that transport is answered with a 404 —
serve resumable sessions over one of the other two.
...
| `#!python on_session` | Called with `#!python (session, origin)` after `#!python session/new` (`#!python "created"`) and `#!python session/load` (`#!python "loaded"`) — see [Loading a session](#loading-a-session) |
...
Two more settings are about what outlives the registry rather than what bounds it —
`#!python storage` and `#!python retain_history`, covered under
...
!!! warning "A hook that cannot answer ends the turn"
    If the hook fails — the approval queue is unreachable, the ticket system is down — or
    simply never answers in time, the turn fails and the client is told so. It is not
    reported to the model as the tool having failed: nobody was asked, so an approval that
    was never requested must not read like a tool that broke, or the model is free to look
    for another way to do the same thing. The gated tool does not run.

    The protocol error carries `#!python data["category"] == "human_input"`, which is what
    to branch on: `#!python data["type"]` is the Python exception name and varies with what
    broke. The hook's own exception is reachable as `#!python exc.cause` in-process.

    To carry on regardless, handle the failure inside the hook and return an answer — a
    denial is usually the right one:
    ...
    async def ask_our_user(event: HumanInputRequest) -> str:
        try:
            ...
            return "n"  # what approval_required() reads as a denial
    ```
...
Reported as **not** supported: `#!python session/resume`, `#!python session/fork`,
`#!python session/list`, `#!python session/delete`, `#!python session/close`, and
connecting to client-provided MCP servers over any transport. The client's own filesystem
and terminal methods are never called either — those the client offers, so there is nothing
to declare.
...
`#!python session/resume` — reattach *without* a replay — is the one most likely to follow
`#!python session/load`; it is not wired yet, and the SDK gates it behind its unstable
protocol flag besides.
