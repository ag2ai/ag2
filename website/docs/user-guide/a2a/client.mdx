...
In the example below, we create a `A2aRemoteAgent` client and use it with a local agent to make them work together.
...
Pretty often you need to pass some custom options to the remote agent HTTP client, such as headers, timeout, etc. To do this, pass your own `HttpxClientFactory` instance to the `A2aRemoteAgent` constructor.
