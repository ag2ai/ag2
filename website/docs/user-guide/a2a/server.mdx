...
[`A2UIAgent`](/docs/user-guide/reference-agents/a2uiagent) is a specialized agent that generates rich user interfaces using the [A2UI protocol](https://a2ui.org/){.external-link target="_blank"}. When wrapped in `A2aAgentServer`, it is auto-detected and the server automatically:

- Uses `A2UIAgentExecutor` to split responses into text + A2UI DataParts
- Declares A2UI v0.9 extension support in the agent card
- Handles extension negotiation — clients that don't request A2UI get text-only responses

```python title="a2ui_server.py" linenums="1" hl_lines="1 6-10"
...
Clients request the A2UI extension via the `X-A2A-Extensions` HTTP header. See the full [A2UIAgent reference](/docs/user-guide/reference-agents/a2uiagent) for details on configuration, validation, actions, and custom catalogs.
