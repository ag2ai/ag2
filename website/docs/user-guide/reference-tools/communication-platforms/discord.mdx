...
executor_agent.initiate_chat(
    ...
    message=[{"content":"Let's send a message to Discord giving them a paragraph on the highlights of Australia.","role":"user"}],
    max_turns=2,
)
```
...
executor_agent.initiate_chat(
    recipient=discord_agent,
    message=[{"content":"Tell me which countries the last two messages on my Discord channel are about and the main differences.","role":"user"}],
    max_turns=2,
)
```
