...
executor_agent.initiate_chat(
    ...
    message=[{"content":"Get the latest weather forecast and send it to our Slack channel. Use some emojis to make it fun!","role":"user"}],
    max_turns=3,
)
```
...
executor_agent.initiate_chat(
    recipient=slack_agent,
    message=[{"content":"Get the last 5 messages about daily weather from our Slack channel and give me a summary of the week's weather.","role":"user"}],
    max_turns=2,
)
```
