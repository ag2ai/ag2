...
By default, node grading is performed one at a time. While this approach is often sufficient, certain applications benefit from grading all nodes in a batch. Batch grading evaluates all available options simultaneously, providing the following advantages:

1. **Context-aware evaluation**: The grader can consider all available options at once, leading to more informed and comparative assessments.
2. **Improved efficiency**: Batch grading reduces the number of LLM calls by combining multiple evaluations into a single request at each step.

To enable batch grading, set `batch_execution=True` in the `reason_config`. By default, `batch_execution` is set to `False`, meaning individual options are graded separately and batch grading is not applied during reasoning.
