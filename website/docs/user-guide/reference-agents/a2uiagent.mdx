...
agent = A2UIAgent(
    ...
    custom_catalog=Path("my_catalog.json"),  # Must include $id field
...
Actions connect UI buttons to behavior. A2UI defines two action types, and `A2UIAction` supports both:
...
The LLM generates buttons with: `{"event": {"name": "schedule_9am", "context": {"time": "9:00 AM"}}}`
...
The LLM generates buttons with: `{"functionCall": {"name": "openUrl", "args": {"url": "https://..."}}}`
