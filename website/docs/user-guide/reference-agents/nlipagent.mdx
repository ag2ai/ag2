...
| Class | Role |
|---|---|
| [`NlipAgentServer`](#server-expose-an-agent-via-nlip) | Wraps a `ConversableAgent` and serves it over HTTP at `/nlip/` |
...
This installs `nlip-sdk`, `nlip-server`, `uvicorn`, and the OpenAI client in one step.

You can also install the optional packages separately:

```bash
pip install nlip-server nlip-sdk uvicorn
```

## Server — Expose a ConversableAgent as an NLIP server

`NlipAgentServer` wraps any `ConversableAgent` as a NLIP HTTP endpoint.
...
from autogen.agentchat.contrib.nlip_agent import NlipAgentServer
...
server = NlipAgentServer(agent, host="0.0.0.0", port=8000)
...
The weather server is an AG2 `ConversableAgent` with two registered tools — `get_weather_alerts` (by US state code) and `get_weather_forecast` (by latitude/longitude) — exposed as an NLIP endpoint via `NlipAgentServer`.
...
server = NlipAgentServer(agent, host="127.0.0.1", port=8014)

if __name__ == "__main__":
    server.run()
```

This file contains a weather agent with two tools: `weather_forecast` and `get_weather_alerts`, which is then exposed as an NLIP server with `NlipAgentServer` on port 8014.
...
async def run_weather_groupchat(task: str, weather_server_url: str = "http://localhost:8014", llm_config=None):
    llm_config = llm_config or {"model": "gpt-4o-mini"}
