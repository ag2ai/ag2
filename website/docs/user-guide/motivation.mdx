...
If you are coming from the earlier `ConversableAgent` / `GroupChat` API, the [network migration guide](/docs/user-guide/network/migration_from_group_chat) maps the classic orchestration patterns onto the new `autogen.network` module.

## How do I try it out?

Install with `pip install ag2`. The framework is importable as the `autogen` module.
...
!!! tip "Using an AI coding assistant?"
    If you build with Claude Code, Cursor, Copilot, or another AI coding assistant, see [Coding with AI Assistants](/docs/user-guide/coding_with_ai) to set it up with AG2 skills and project rules so it writes against the current `autogen` API.
