...
The terminal states are immutable — once set, further `#!python complete() / fail() / progress()` calls are silent no-ops. The set is exported as `#!python autogen.task.TERMINAL_TASK_STATES`.
...
from autogen.events import TaskCompleted, TaskProgress, TaskStarted
...
from autogen.task import TaskInject
