...
```python linenums="1"
from autogen.network import FullTranscript, WindowedSummary
...
```python linenums="1"
from autogen.network import resolve_view_policy
...
The `#!python BaseEvent` types you emit determine how the LLM sees the history: `#!python ModelRequest` for messages "from the user," `#!python ModelMessage` for messages "from the assistant," and so on. Look at `#!python autogen.events` for the full taxonomy.

## CompactionSummary

When `#!python WindowedSummary` elides envelopes outside its window, it prepends a `#!python CompactionSummary(text="...elided N turns")` event. This is from `autogen.compact` — the LLM sees it as a system-supplied note that "there is earlier history I'm not showing you." This keeps the LLM's behaviour grounded in long-running discussions without exploding the token budget.
...
```python linenums="1"
from autogen.network import parse_skill_frontmatter, ParsedSkill
...
```python linenums="1"
from autogen.network import render_fallback_skill
