...
| **An existing CLI login** on the host (Claude Code `~/.claude`, `opencode auth login`, `kilo auth login`) | Nothing to set — inherited via `$HOME` | Whatever **plan that login is on**, which may be a **subscription** |
...
!!! note
    ...
    `CLAUDE_CONFIG_DIR` passed in `env`), OpenCode (`opencode auth login`) and
    Kilo Code (`kilo auth login`). Exact
