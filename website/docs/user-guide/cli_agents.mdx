...
agent = Agent(
    "dev",
    config=ClaudeCodeConfig(cwd="./repo", permission_policy="auto"),
    tools=[get_ticket],
)
...
- The MCP server binds to `127.0.0.1` on a random port and lives exactly as
  long as the ACP session. Requests are validated against `Host`/`Origin`
  allowlists (DNS-rebinding protection); there is no authentication beyond
  that, so other local processes on the same machine can reach it.
