...
from autogen.eval import scorer
...
Six ship under `autogen.eval.scorers`. Four are simple, deterministic checks (below); two richer ones — `agent_judge` and `failure_attribution` — get their own sections after.
...
```python linenums="1"
from autogen.eval import Scorer, Trace
