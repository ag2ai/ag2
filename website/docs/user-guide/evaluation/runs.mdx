...
```python linenums="1"
from autogen.eval import Suite
...
```python linenums="1"
from autogen.eval import BudgetThresholds, run_agent
...
```python linenums="1"
from autogen.eval import DirectoryTraceSource, evaluate_traces
...
from autogen.eval import run_variants
from autogen.eval.events import VariantCompleted
...
The events live in `autogen.eval.events` and are all transient — observational only, since the durable record of a run is its persisted JSON:
...
!!! tip "Ready-made console output"
    For a quick console view without writing a callback, subscribe the built-in `console_reporter` (from `autogen.eval`): `#!python stream.subscribe(console_reporter)`. It's just an opt-in observer — no `run_agent()` flag — so swap in your own callback whenever you want different output.
