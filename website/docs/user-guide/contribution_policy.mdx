...
!!! note
    This policy applies to AG2 (`autogen`), which becomes the official AG2 framework at v1.0. See the [Roadmap](/docs/user-guide/roadmap) for the full v1.0 plan.
...
Today, everything shipped under `autogen` is Core: the Agent runtime, primary LLM providers, common built-in tools, middleware, structured output, telemetry, history management, and the context and dependency-injection primitives.
...
Core and Extensions live in the same repository. Extensions go under `autogen/extensions/` and ship as part of `pip install ag2`. The contribution flow stays familiar during the transition.
