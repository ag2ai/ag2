...
- Python backend runtime with an AG-UI endpoint (for example, `/chat` from [AG-UI integration](./))
- Node.js and a Next.js frontend
- CopilotKit + AG-UI client packages in your frontend:
  `@copilotkit/react-core`, `@copilotkit/runtime`, `@ag-ui/client`, `zod`
...
This guide uses CopilotKit v2 hooks/components from `@copilotkit/react-core/v2`. The `<CopilotKit>` provider is imported from `@copilotkit/react-core`.
...
export function OrderStatusTool() {
  useFrontendTool({
    ...
    parameters: z.object({
      ...
      status: z.string().describe("Order status text"),
    }),
    handler: async ({ orderId, status }) => JSON.stringify({ orderId, status }),
    render: ({ args, status: toolStatus, result }) => {
      if (toolStatus === ToolCallStatus.InProgress) {
        return <div>Loading order status...</div>;
      }
      const payload = result ? JSON.parse(result) : args;
      return (
        <div>
          <strong>Order {payload.orderId}</strong>
...
- CopilotKit v2 hooks/components in this guide import from `@copilotkit/react-core/v2` (for example `useAgent`, `useFrontendTool`, `CopilotSidebar`).
