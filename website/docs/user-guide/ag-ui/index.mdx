...
[`A2UIAgent`](/docs/user-guide/reference-agents/a2uiagent) extends `AGUIStream` with support for the [A2UI protocol](https://a2ui.org/){.external-link target="_blank"}, enabling agents to generate structured UI components (cards, forms, buttons, images) instead of plain text.

The agent works with AG-UI through an **event interceptor** that extracts A2UI JSON from responses and emits `ActivitySnapshotEvent` events:
...
Frontend renderers like CopilotKit's `@copilotkit/a2ui-renderer` consume the `ACTIVITY_SNAPSHOT` events and render the UI components. See the full [A2UIAgent reference](/docs/user-guide/reference-agents/a2uiagent) for configuration, validation, actions, and A2A integration.
