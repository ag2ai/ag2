...
Run the channel process alongside the AG-UI server from the [basic server example](/docs/user-guide/ag-ui/overview#basic-server-example). Both credentials come from the Intelligence dashboard — the API key under **API Keys**, the Code on the Channel you created there:
...
1. Build the AG-UI endpoint from the [basic server example](/docs/user-guide/ag-ui/overview#basic-server-example) if you have not already.
