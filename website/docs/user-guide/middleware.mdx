...
It preserves the first `ModelRequest` when possible, and drops whatever the cut orphaned so the trimmed history still replays.

`max_events` is a target rather than a hard cap, for the same reason it is one on the reduction policies — see [Reduction limits are targets](/docs/user-guide/advanced/assembly#reduction-limits-are-targets).
...
`max_tokens` is a target rather than a hard cap — see [Reduction limits are targets](/docs/user-guide/advanced/assembly#reduction-limits-are-targets).
