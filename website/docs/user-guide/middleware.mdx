...
Middleware lets you intercept and customize how an **AG2** agent runs a turn.
...
For wrapping **one** tool at definition time with async hooks, see [Tool middleware](/docs/user-guide/tools/tool_middleware).
...
from autogen.middleware import LoggingMiddleware, RetryMiddleware
...
from autogen import Agent
from autogen.config import OpenAIConfig
from autogen.middleware import LoggingMiddleware, TokenLimiter
...
from autogen.middleware import AgentTurn, BaseMiddleware
...
from autogen.middleware import BaseMiddleware, LLMCall, Middleware, ToolExecution
...
AG2 currently includes four built-in middleware in `autogen.middleware`:

### `LoggingMiddleware`

```python linenums="1"
from autogen import Agent
from autogen.middleware import LoggingMiddleware
...
from autogen.middleware import ConditionalMiddleware, Middleware
