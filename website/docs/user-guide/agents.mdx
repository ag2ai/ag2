...
!!! tip "Building with an AI coding assistant?"
    See [Coding with AI Assistants](/docs/user-guide/coding_with_ai) to set up Claude Code, Cursor, Copilot, or another assistant with AG2 skills and project rules so it writes against the current `autogen` API.
