...
| `on_tool_execution` | Pre-tool defense | Tool allowlist/blocklist, argument validation, PII scan, secret scan, cost limit check, rate limit |
...
All three stages activate from a single middleware instance. The **middleware factory** pattern keeps state (decisions, receipts, cost, frozen agents, rate-limit windows) alive across turns.
...
|------|----------|----------|
| **ENFORCE** | Evaluates all policies. Blocks violations with `ToolErrorEvent`, and holds calls that a rate limit is throttling. | Production |
| **MONITOR** | Evaluates all policies, records decisions, but **never blocks** — and never waits, since a wait blocks too. | Staging / shadow testing |
| **OBSERVE** | Skips policy evaluation. Passes through, still tracks cost and records an `OBSERVE_PASSTHROUGH` decision, but does not fill rate-limit windows. | Initial rollout / lowest overhead |
...
Cap how fast tool calls happen within a rolling time window — the defence against a runaway loop that keeps calling a tool and burns provider rate limits, side effects, and budget:

```python linenums="1"
# Global: throttle to 30 tool calls per minute, across all tools
GovernancePolicy.rate_limit(30, window_seconds=60)
# Per-tool: at most 5 emails per minute, refused rather than delayed
GovernancePolicy.rate_limit(5, window_seconds=60, tool="send_email", on_exceeded="deny")
# Pattern: the whole db_* family shares one budget, held at most 30s
GovernancePolicy.rate_limit(100, window_seconds=3600, tool="db_*", max_wait_seconds=30)
```

It is a **sliding window**: only the calls in the last `window_seconds` count, so the allowance refills continuously rather than resetting on a fixed boundary. Omit `tool` for a global limit that every governed call shares; pass a tool name or `fnmatch` pattern to scope the limit to matching calls. Combine several — a generous global ceiling plus tighter per-tool caps — and an over-limit call waits for (or is refused by) the first limit it trips.

What happens at the limit is `on_exceeded`:

| `on_exceeded` | Behaviour |
|---|---|
| `"wait"` (default) | The call is **held** until the window has room, then runs. Nothing is denied. |
| `"deny"` | The call is **refused** with `RATE_LIMIT_EXCEEDED` and risk score 70. |

!!! warning "Prefer waiting — a refusal can cost more than the call it blocked"

    A refused call goes back to the model as a tool error, and an agent stuck in a loop answers it by calling again. Each refusal therefore spends a model round-trip, so refusing a runaway loop burns tokens *faster* than the tool calls it replaced — while stopping only the tool-side damage. Holding the call actually slows the loop to the configured rate, which is what the limit is for.

    Reach for `on_exceeded="deny"` where a late call is worse than no call, or where stalling the turn is unacceptable.

A wait is bounded by `max_wait_seconds`, which defaults to `window_seconds` — the longest a sliding window can ever need before a slot frees. A call facing a longer wait is denied instead of held, so a turn cannot stall indefinitely. Each wait is recorded on the decision as `RATE_LIMIT_WAIT:{seconds}s`, so throttling shows up in the audit trail rather than as unexplained latency.

Only calls that actually ran count toward the window, so a call blocked in `ENFORCE` never consumes the budget. In `MONITOR` a call over the limit is recorded as a denial but still executes — and therefore still counts, so the window keeps measuring real load; `MONITOR` never waits, since waiting is itself a form of blocking. `OBSERVE` skips policy evaluation entirely and records nothing, so windows stay empty in that mode.

`max_calls` must be a positive integer, and `window_seconds` and `max_wait_seconds` positive numbers, validated at construction so a policy that could never allow a call fails where it is written.

Wired into an agent, a generous global ceiling and a tight per-tool cap compose — the agent is throttled to 30 tool calls a minute overall, while a sixth email in the same minute is refused outright rather than delayed:
...
def send_email(to: str, body: str) -> str:
    """Sends an email and returns the delivery status."""
    return f"sent to {to}"

agent = Agent("assistant", tools=[send_email], middleware=[governance])
```

!!! note "Window state lives on the middleware instance"

    Call timestamps are held on the `TealTigerMiddleware` factory and shared across turns for the life of that instance (cleared by `reset()`). A limit therefore spans a whole session, not a single turn — but it does not persist across process restarts or share state between separate middleware instances, so it cannot protect a provider quota shared by several workers.

    Timing uses a monotonic clock, so a window is unaffected by wall-clock changes such as an NTP correction.
...
- **ENFORCE mode**: a frozen agent's turn raises `GovernanceDeniedError` (a `PermissionError` carrying the `decision`) before the model is called; a tool call returns `ToolErrorEvent`
...
| `reset()` | Clear decisions, receipts, cost, frozen agents, and rate-limit windows |
