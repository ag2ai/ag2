...
!!! note "Use `code_environment()` for `SandboxCodeTool`"
    ...
    Tenki's image ships `python3`, not `python`, so `SandboxCodeTool`'s default interpreter mapping does not apply. `env.code_environment()` returns a `CodeAdapter` with the right runner already wired; pass `languages=` to it to restrict what the tool accepts. `SandboxShellTool` takes the environment directly — it runs whatever command it is given.
