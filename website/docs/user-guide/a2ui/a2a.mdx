...
A2UI's wire is transport-agnostic, so the same validated A2UI messages can ride an [A2A](/docs/user-guide/a2a/) exchange instead of the built-in [REST adapter](/docs/user-guide/a2ui/server). The `#!python autogen.a2ui.a2a` submodule wraps a plain `#!python autogen.Agent` in an `#!python A2UIAgentExecutor`, plugs it into a standard `#!python A2AServer`, and carries each turn's A2UI messages as a canonical A2A `#!python DataPart`.
...
from autogen.a2ui.a2a import A2UIAgentExecutor, get_a2ui_agent_extension
