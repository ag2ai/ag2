...
2. Compatibility with `ConversableAgent`: Now you can directly use `ConversableAgent` or `AssistantAgent` in the swarm chat. We recommend switching to `AssistantAgent` since `SwarmAgent` might be deprecated in the future.
...
3. **Start Swarm Chat**: initiate the swarm chat with `initiate_swarm_chat` or `a_initiate_swarm_chat` for asynchronous calls.
...
We pass in a user agent to the swarm chat to accept user inputs. With `agent_6`, we register an `AfterWork` handoff to revert to the user agent when no tool calls are suggested.
