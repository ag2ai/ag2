...
2. Compatibility with `AssistantAgent`: Now you can directly use `AssistantAgent` in the swarm chat. We recommend switching to `AssistantAgent` since `SwarmAgent` might be deprecated in the future.
3. Deprecation Warning: `OnCondition` and `AfterWork` will be deprecated in the future. Please use [`OnCondition`](/docs/api-reference/autogen/OnCondition) and [`AfterWork`](/docs/api-reference/autogen/AfterWork) instead.
...
Learn about the high-level concepts in [Basic Concepts](/docs/user-guide/basic-concepts/swarm) section.
...
1. **Create Agents**: instantiate an `AssistantAgent` to be part of the swarm chat.
    - API Ref: [AssistantAgent](/docs/api-reference/autogen/AssistantAgent)
2. **Register Handoffs**: utilize a standalone `register_hand_off` to register `OnCondition` and `AfterWork` handoffs.
    - API Ref: [register_hand_off](/docs/api-reference/autogen/register_hand_off),[OnCondition](/docs/api-reference/autogen/OnCondition), [AfterWork](/docs/api-reference/autogen/AfterWork), [AfterWorkOption](/docs/api-reference/autogen/AfterWorkOption), [SwarmResult](/docs/api-reference/autogen/SwarmResult)
3. **Update Agent State (Optional)**: update an agent's state before replying.
    - API Ref: [UPDATE_SYSTEM_MESSAGE](/docs/api-reference/autogen/UPDATE_SYSTEM_MESSAGE)
4. **Start Swarm Chat**: initiate the swarm chat with `initiate_swarm_chat` or `a_initiate_swarm_chat` for asynchronous calls.
    - API Ref: [initiate_swarm_chat](/docs/api-reference/autogen/initiate_swarm_chat), [a_initiate_swarm_chat](/docs/api-reference/autogen/a_initiate_swarm_chat)


### Create Agents

You can directly create `AssistantAgent` to be used in a swarm chat. Instead of registering functions one by one, you can pass in a list of functions to `AssistantAgent(functions=[func1, ...])`. These functions will be converted to schemas to be passed to the LLMs, and you don't need to worry about registering the functions for execution.
...
- You can pass back a `SwarmResult` class, where you can return a value, the next agent to call, and update context variables at the same time.
...
It can be useful to update an agent's state before they reply. For example, using an agent's context variables you could change their system message based on the state of the workflow.
...
Below is an example of setting these up when creating an agent.
