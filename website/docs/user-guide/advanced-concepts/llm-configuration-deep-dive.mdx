...
## Introduction to the LLMConfig argument

Different tasks may require different models, and the configuration allows specifying the different endpoints and configurations that are to be used. The argument is a list of dictionaries, each of which contains the following keys depending on the kind of endpoint being used:
...
A common, useful pattern used is to define this configuration via JSON (specified as a file or an environment variable set to a JSON-formatted string) and then use the `from_json` method to load it:
...
Model names can differ between OpenAI and Azure OpenAI, so tags offer an easy way to smooth over this inconsistency. Tags are a list of strings in the configuration list. For example, for the following configuration list:
...
When filtering the configuration list, you can pass a single tag or multiple tags. A config is selected if it has at least one of the specified tags:
...
Besides the main list, there are other parameters that can be used to configure the LLM. These are split between parameters specifically used by Autogen and those passed into the model client.
