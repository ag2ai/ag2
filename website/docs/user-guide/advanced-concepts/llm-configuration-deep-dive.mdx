...
In AG2, agents use LLMs as key components to understand and react. <Tip>
**AG2 supports configuring LLM access via dedicated environment variables _in priority_, before falling back to `OAI_CONFIG_LIST`.**

**Recognized environment variables (preferred defaults):**
- `OPENAI_API_KEY` (OpenAI)
- `AZURE_OPENAI_API_KEY` and `AZURE_OPENAI_API_BASE` (Azure OpenAI)
- `GEMINI_API_KEY` (Google Gemini)
- `ANTHROPIC_API_KEY` (Anthropic)
- `DEEPSEEK_API_KEY` (DeepSeek)
- `CEREBRAS_API_KEY` (Cerebras)
- (other integrations, e.g., Bedrock or Ollama, may require their own variables)
</Tip>

AG2 _first tries_ to build a config from any of those environment variable keys present in your environment. Only if none are found, will it use `OAI_CONFIG_LIST`, which can be provided either as a file path or a JSON string in an environment variable.

To configure an agent's access to LLMs, you can specify an `llm_config` argument in its constructor. For example, the following snippet shows a configuration that uses `gpt-4.1` via the recommended environment variable pattern:
...
<Warning>
It is important to **never commit secrets into your code**, therefore you should always read API keys from environment variables, such as `OPENAI_API_KEY`, `AZURE_OPENAI_API_KEY`, etc.
</Warning>
...
- `api_key` (str, optional): The API key required for authenticating requests to the model's API endpoint. _Preferably reference `os.environ["OPENAI_API_KEY"]`._
...
- `api_key` (str, optional): The API key required for authenticating requests to the model's API endpoint. _Preferably reference `os.environ["AZURE_OPENAI_API_KEY"]`._
- `api_type`: `azure`
- `base_url` (str, optional): The base URL of the API endpoint. This is the root address where API calls are directed.
...
- `api_key` (str, optional): The API key required for authenticating requests to the model's API endpoint. See your provider docs for the correct environment variable.
- `base_url` (str, optional): The base URL of the API endpoint. This is the root address where API calls are directed.
- `tags` (List[str], optional): Tags which can be used for filtering.
...
<Tip>
**Tip:** These environment variables are checked _before_ any config files or JSON lists (`OAI_CONFIG_LIST`). Only if the specific env vars are missing, AG2 will search for and use `OAI_CONFIG_LIST`.
</Tip>

### `OAI_CONFIG_LIST` pattern (Fallback)
...
This is the _fallback_ behavior if the AG2-supported env vars above are not set.
...
This can then be applied to a config when constructing an `LLM_CONFIG` with the `where` method:
...
When filtering the `config_list`, you can pass a single tag or multiple tags. A config is selected if it has at least one of the specified tags:
