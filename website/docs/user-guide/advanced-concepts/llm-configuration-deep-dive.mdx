...
### Modern AG2 `config_list` pattern

The current recommended pattern is to construct the `config_list` as a Python list in memory, typically with model info and API keys loaded from environment variables, without relying on `from_json` or external files. For example:
...
You can use this list directly, and filter for specific models or tags as shown below.

### Filtering config_list

The list can be filtered based on criteria such as model or tags by using the `where` method:
