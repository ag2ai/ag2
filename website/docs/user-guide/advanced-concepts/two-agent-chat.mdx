...
A two-agent chat takes two inputs: a message, which can be a string (for convenience), a dictionary representing a message, **or a list of message dicts** (representing a full conversation history), and a context, which specifies various parameters of the chat.
