...
```mermaid
flowchart TD
    User([User]) --> |"Reports an issue"| GroupOrch[Group Chat Orchestration]
    GroupOrch --> |"Passes to initial agent"| Triage[Triage Agent]
    Triage --> |"Determines it's a technical issue"| Tech[Tech Agent]
    Tech --> |"Handles technical issue"| Triage

    Triage --> |"Determines it's a general issue"| General[General Agent]
    General --> |"Provides general help"| Triage

    Triage --> |"Doesn't specify next agent"| GroupManager[Group Manager]
    GroupManager ...->  Tech
    GroupManager ...->  Human
    GroupManager ...->  General
...
- Handoffs – Learn how to control and customize transitions between agents, both implicitly (via context) and explicitly (via agent commands)
- Context Variables – Understand how to share and retain state across agents during a conversation
- Advanced Features – Discover capabilities like context-aware routing, nested group chats, and dynamic agent creation

Group Chat provides a powerful framework for building sophisticated multi-agent systems that solve complex problems through clear role division and smooth collaboration.
