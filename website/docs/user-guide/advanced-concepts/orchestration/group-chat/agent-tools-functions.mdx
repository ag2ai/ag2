...
## Why Tools Matter in Group Chat
...
- **Workflow Control** - Tools help direct the conversation flow, and can determining which agent speaks next
...
- **context_variables**: Updated shared state (we'll explore this in the next section)

This simple but powerful structure allows tools to both communicate results and influence the conversation flow.

## Implementing Basic Tools in Group Chat
...
By adding the `classify_query` function to the triage agent, we've given it a specific tool to use for routing queries instead of relying solely on its LLM capabilities. The function returns a `ReplyResult` that specifies which agent should receive control next.

### Enhancing Tools with Type Annotations

We can improve our tool using type annotations to make it more self-documenting and provide better guidance to the LLM:
...
<Note>
If you run the example in a non-docker environment, you will see the below error in your console. It is recommended to run the example in a docker environment. It that's not possible, you can set the environment variable `AUTOGEN_USE_DOCKER` to `0/False/no` or set the `use_docker` parameter in the `code_execution_config` to `False`.
...
Understanding context variables will then provide the foundation for mastering Handoffs and Transitions, which gives you complete control over the flow of conversation between agents.
