...
- [`FunctionTarget`](/docs/api-reference/autogen/agentchat/group/targets/function_target/FunctionTarget): Transfer control to a custom function that can run logic before determining the next transition
...
It can perform any custom logic, and then returns a [`FunctionTargetResult`](/docs/api-reference/autogen/agentchat/group/targets/function_target/FunctionTargetResult) that specifies the next transition target and includes a message for the next agent.
