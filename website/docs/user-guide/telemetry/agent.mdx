...
<<<<<<<< HEAD:website/docs/user-guide/telemetry/agent.mdx
AG2 Beta ships a `#!python TelemetryMiddleware` that emits [OpenTelemetry](https://opentelemetry.io/){.external-link target="_blank"} spans for a single agent's work — its turns, LLM calls, tool executions, and human-in-the-loop interactions.
...
>>>>>>>> origin/main:website/docs/user-guide/telemetry.mdx
