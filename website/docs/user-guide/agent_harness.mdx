...
| `include_tools` | Allowlist of parent function-tool names to inherit. `None` means "inherit all". A builtin tool or a `Toolkit` has no function name, so an allowlist drops it. |
| `exclude_tools` | Blocklist of parent function-tool names to drop. Applied after `include_tools`. |
| `extra_tools` | Additional tools given to sub-tasks that the parent does not have. |
| `max_concurrency` | Optional cap shared by all sub-tasks spawned by this Agent. Extra work waits for a slot. `None` keeps concurrency unbounded. |
...
agent = Agent(
    ...
    tasks=TaskConfig(
        ...
        max_concurrency=4,                    # bound model/API fan-out
    ),
)
```
...
Set `TaskConfig(max_concurrency=N)` when that fan-out needs a model, API, or
resource limit. The cap is shared by both tools on the Agent, so several
parallel `run_subtask` calls and a concurrent `run_subtasks` call cannot
bypass one another.
