...
# We associate them with the agents using [`register_for_execution`](/docs/api-reference/autogen/ConversableAgent#register-for-execution) for the user_proxy so it can execute the function and [`register_for_llm`](/docs/api-reference/autogen/ConversableAgent#register-for-llm) for the chatbot (powered by the LLM) so it can pass the function definitions to the LLM.
