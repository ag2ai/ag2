...
If you want to use the OpenAI Responses API, you can specify the `api_type` as `responses`.
Note that the `responses` API is not supported for group chat for now.
