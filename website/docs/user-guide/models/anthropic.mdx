...
- Streaming (just set `"stream": True` in your config for real-time output with supported models)
- Structured Outputs ([Notebook example](/docs/use-cases/notebooks/notebooks/agentchat_structured_outputs))
- Token usage and cost computed exactly per Anthropic's current API costs
...
## Requirements
To use Anthropic Claude with AG2, install the `ag2[anthropic]` package. As of 2024, structured outputs, streaming, and tools require `anthropic[vertex]>=0.79.0`.

```bash
# If you need to install AG2 with Anthropic (new models, tools, streaming, and structured output)
...
[
    {
        "model": "claude-sonnet-4-5",  # use latest model names
        "api_key": "your api key",
        "api_type": "anthropic",
        "stream": True,                # enable streaming if you want real-time output
    },
    {
        "model": "claude-haiku-4-5",  # smaller faster model
        "api_key": "your api key",
        "api_type": "anthropic",
    },
    {
        "model": "claude-opus-4",     # top-tier Anthropic model
        "api_key": "your api key",
        "api_type": "anthropic",
        "max_tokens": 10000,
    },
...
Construct a simple conversation between a User proxy and an ConversableAgent based on Claude models.
...
```
... (no change to example output and additional narrative; only model and streaming mentions updated as above) ...

## Tool Call Example with the Latest Anthropic API
Anthropic announced that tool use and structured outputs are now generally available for supported models. You must use `anthropic[vertex]>=0.79.0` or newer.
...
```
... (no further change in narrative required here) ...

## ... (remaining group chat, thinking mode, and advanced usage sections unchanged except where model names and config keys updated for consistency with new Anthropic versions and conventions) ...
