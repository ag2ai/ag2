...
!!! note
    This page covers **local** skills that run on your machine. To activate skills hosted and executed by the LLM provider (Anthropic or OpenAI server-side), use `SkillsTool` — see [Provider Skills](/docs/user-guide/tools/builtin_tools#provider-skills).
...
The body accepts a **callable** as well as a string. Pass one and it is rendered on every `load_skill` instead of being fixed when the skill is constructed — so the instructions the model reads can reflect the live run. Register it with the `@skill.instructions` decorator, alongside the resource and script decorators:
...
deploy = MemorySkill(
    name="deploy",
    description="Deploy the service. Use when asked to ship or roll out a release.",
)

@deploy.instructions
async def deploy_body(region: Annotated[str, Variable("region")]) -> str:
    return f"Deploy to {region}. Run the health check before switching traffic."
```

The callable may be sync or async, and supports the same dependency injection as a resource or script. Use it when the *always-needed* part of the instructions varies; a resource still fits better for bulk data the model should fetch only when it turns out to need it.

A callable can also go straight to the constructor — `MemorySkill(..., instructions=deploy_body)` — which is handy when the same body is shared by several skills. When both are given, the decorator wins.

!!! note
    Only the body is dynamic. A skill's catalog entry — the name and description in `<available_skills>` — is a snapshot taken when the plugin is built, so it stays in lockstep with the set of skills the activation tools accept.
