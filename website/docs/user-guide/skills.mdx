...
`instructions` accepts a **callable** as well as a string. Pass one and the body is rendered on every `load_skill` instead of being fixed when the skill is constructed — so the instructions the model reads can reflect the live run:
...
async def deploy_instructions(region: Annotated[str, Variable("region")]) -> str:
