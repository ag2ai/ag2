...
AG2 Classic is published on PyPI as the `autogen` package. Install it with:
```bash
pip install autogen
```

!!! warning
    Install `autogen`, **not** `ag2`. The `ag2` package now refers to AG2 v1.0+, which is a separate, newer release. AG2 Classic only ships under the `autogen` name.
...
- **Recommended**: Install the `docker` optional extra with `pip install autogen[docker]`, and make sure the Docker daemon is up and running.
