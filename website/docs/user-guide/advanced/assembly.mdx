...
!!! tip
    `AssemblerMiddleware` and `AssemblyPolicy` live in `#!python autogen.assembly` if you need to wire the middleware manually (for example, inside a custom harness that isn't built on `Agent`).
...
from autogen.policies import AlertPolicy, SlidingWindowPolicy
...
All six built-ins are importable from `#!python autogen.policies`.
...
```python linenums="1"
from autogen.policies import ConversationPolicy
...
```python linenums="1"
from autogen.policies import TokenBudgetPolicy
...
from autogen.assembly import AssemblerMiddleware
from autogen.policies import WorkingMemoryPolicy
...
```python linenums="1"
from autogen.policies import EpisodicMemoryPolicy
