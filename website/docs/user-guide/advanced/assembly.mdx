...
Keeps the last `max_events` events, minus whatever the cut orphaned — see [Reduction limits are targets](/docs/user-guide/advanced/assembly#reduction-limits-are-targets).
...
Set `transparent=True` to append a prompt note like `"[sliding_window] Showing last 50 of 123 events."` — useful while tuning. The count in that note is the window actually sent, so it is the cheapest way to see the target being missed.
...
The budget is a target for the same reason the window is — see [Reduction limits are targets](/docs/user-guide/advanced/assembly#reduction-limits-are-targets).
...
## Reduction limits are targets

`max_events` and `max_tokens` are targets, not guarantees. A reduction policy cuts history at an index and sends everything after it, and a provider only accepts that tail if it can stand on its own. Three retained events cannot:

| Retained event | Needs | If the cut dropped it |
|---|---|---|
| a tool result | the call it answers | orphaned `function_call_output` |
| a builtin (server-side) call | its reasoning item | orphaned `web_search_call` |
| a bare call event | the `ModelResponse` announcing it | maps to no provider item at all |

Anything left orphaned is dropped from the window, so the retained span can come out **smaller** than the limit. And if dropping the orphans would empty the span, the cut retreats until the span holds something sendable — so a limit narrower than the turn it lands in comes out **larger** than asked. Overshooting a limit is recoverable; an empty request is rejected outright.

!!! note
    This is why a reduction policy can appear to ignore its limit on turns that use builtin tools: one reasoning item anchors the whole group of server-side calls behind it, and that group only replays intact. Set `transparent=True` to see the size actually sent.
