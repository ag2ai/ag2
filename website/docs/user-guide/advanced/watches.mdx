...
A **Watch** is a reactive trigger primitive. You arm it on a [Stream](/docs/user-guide/advanced/stream), and it fires a callback when its condition is met — where "condition" can be an event match, a count, a time window, a schedule, or a composition of other watches.

Watches are the mechanism behind trigger-driven [Observers](/docs/user-guide/advanced/observers), but they can be used standalone for custom reactive logic on any Stream.
...
Every Watch implements the same `Watch` protocol (importable from `#!python autogen`):
...
```python linenums="1"
from autogen.watch import DelayWatch
...
```python linenums="1"
from autogen.watch import IntervalWatch
...
```python linenums="1"
from autogen.watch import CronWatch
...
```python linenums="1"
from autogen.watch import AllOf, EventWatch
...
```python linenums="1"
from autogen.watch import AnyOf, EventWatch
...
```python linenums="1"
from autogen.watch import Sequence, EventWatch
...
A Watch callback can send events back onto the stream — this is how [trigger-driven Observers](/docs/user-guide/advanced/observers#trigger-driven-observers-baseobserver) emit alerts:

```python linenums="1"
from autogen import MemoryStream
from autogen.watch import DelayWatch
from autogen.events import ObserverAlert, Severity
...
Most of the time you won't call `arm()` / `disarm()` directly — you'll hand the Watch to a [BaseObserver](/docs/user-guide/advanced/observers#trigger-driven-observers-baseobserver), which manages the lifecycle against the agent's stream.

## Next steps

- Use Watches inside a [BaseObserver](/docs/user-guide/advanced/observers#trigger-driven-observers-baseobserver) for agent-scoped monitoring.
- See [Stream](/docs/user-guide/advanced/stream) for event publishing and subscription mechanics.
