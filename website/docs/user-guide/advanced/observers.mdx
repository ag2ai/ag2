...
Observers let you attach lightweight, read-only event listeners directly to an Agent. Under the hood, each observer is a regular [Stream subscriber](/docs/user-guide/advanced/stream) — but you register it on the Agent instead of managing the stream yourself.

Use observers when you want to monitor agent behavior (logging, metrics, debugging) without writing a full [Middleware](/docs/user-guide/middleware) class.
...
Observers support the same condition system as [stream subscriptions](/docs/user-guide/advanced/stream). You can filter by event type, combine types, or match on field values:

```python linenums="1" hl_lines="4 9 14 19"
from autogen.events import ModelRequest, ModelResponse, ToolCallEvent
...
Observer callbacks support the same dependency injection features as [Agent Tools](/docs/user-guide/tools/tools/) and [stream subscribers](/docs/user-guide/advanced/stream/) — `Context`, `Inject`, `Depends`, and `Variable`:

```python linenums="1" hl_lines="5"
from autogen import Context, observer
from autogen.events import ModelResponse
...
A `BaseObserver` is an ABC that pairs a [Watch](/docs/user-guide/advanced/watches) with a `process()` method. The Watch decides *when* to fire; `process()` decides *what to do* with the collected events. If `process()` returns an `ObserverAlert`, the base class emits it back onto the stream for other subscribers to consume.
...
```python linenums="1"
from autogen import Agent
from autogen.observer import LoopDetector
from autogen.config import OpenAIConfig
...
```python linenums="1"
from autogen import Agent
from autogen.observer import TokenMonitor
from autogen.config import OpenAIConfig
...
from autogen.events import ObserverAlert
...
!!! note
    `ObserverAlert` is emitted to the stream and persisted in history, but **is not rendered back to the LLM** by the default provider mappers. If you want the agent itself to react to alerts, write a [Middleware](/docs/user-guide/middleware) that converts alerts into follow-up messages.

Severity levels live in `autogen.events.Severity`: `INFO`, `WARNING`, `CRITICAL`, `FATAL`.

### Building a custom observer

Subclass `BaseObserver`, pick a [Watch](/docs/user-guide/advanced/watches), implement `process()`:

```python linenums="1" hl_lines="10 13 17-21"
from autogen import Context
from autogen.observer import BaseObserver
from autogen.watch import CadenceWatch
from autogen.events import BaseEvent, ModelResponse, ObserverAlert, Severity
