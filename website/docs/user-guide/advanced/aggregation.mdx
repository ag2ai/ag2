...
```python linenums="1"
from autogen.aggregate import AggregateTrigger
...
Both built-ins are importable from `#!python autogen.aggregate` and both take a `#!python ModelConfig` for a summarization LLM call. Use a smaller / cheaper model than the agent's main model.
...
from autogen.aggregate import ConversationSummaryAggregate
...
```python linenums="1" hl_lines="6 9"
from autogen.aggregate import WorkingMemoryAggregate
from autogen.config import OpenAIConfig
from autogen.knowledge import MemoryKnowledgeStore
...
The path constants (`WORKING_MEMORY_PATH`, `CONVERSATIONS_PREFIX`) are defined in `#!python autogen.knowledge`. Both sides of each pair use those constants so the producer/consumer contract is held together by types, not magic strings.
...
from autogen import Agent, KnowledgeConfig
from autogen.aggregate import AggregateTrigger, ConversationSummaryAggregate
from autogen.config import OpenAIConfig
from autogen.knowledge import MemoryKnowledgeStore
