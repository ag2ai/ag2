...
<Tip>
A field is declared with the type of its value, so a type checker reads
`ToolCallEvent.name == "fetch_data"` as a `bool` unless it is told otherwise. AG2 ships a
mypy plugin that makes class-level field access resolve to the field descriptor, whose
comparison operators return a `Condition`. Add it to your own configuration to check these
filters in your project:
