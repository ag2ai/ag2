...
<div style={{ display: "inline-block", marginRight: "0px", height: "20px" }}>
  <a
    href="https://colab.research.google.com/github/ag2ai/ag2/blob/main/website/docs/tutorial/conversation-patterns.mdx"
    target="_blank"
...
<div style={{ display: "inline-block", height: "20px" }}>
  <a
    href="https://github.com/ag2ai/ag2/blob/main/website/docs/tutorial/conversation-patterns.mdx"
    target="_blank"
    style={{ height: "20px", display: "inline-flex", alignItems: "center" }}
  >
    <img
...
3. **Group Chat**: a single chat involving more than two agents. An important question in group chat is: What agent should be next to speak? To support different scenarios, we provide different ways to organize agents in a group chat:
   ...
   - We allow you to pass in a function to customize the selection of the next speaker. With this feature, you can build a **StateFlow** model which allows a deterministic workflow among your agents.
     Please refer to this [guide](/website/docs/topics/groupchat/customized_speaker_selection) and this [blog post](/blog/2024/02/29/StateFlow) on StateFlow for more details.
...
![Two-agent chat](./assets/two-agent-chat.webp)
...
chats between two agents, chained together by a mechanism called _carryover_,
...
![initiate_chats](./assets/sequential-two-agent-chat.webp)

In this pattern, the a pair of agents first start a two-agent chat,
then the summary of the conversation becomes a _carryover_ for the next
...
![group_chat](./assets/group-chat.webp)
...
![nested_chat](./assets/nested-chats.webp)
...
[`register_reply`](/website/docs/reference/agentchat/conversable_agent/#register_reply)
...
You can hide [tool usages](/website/docs/tutorial/tool-use) within a single agent by having the tool-caller agent
starts a nested chat with a tool-executor agent and then use the result
of the nested chat to generate a response.
See the [nested chats for tool use notebook](/notebook/agentchat_nested_chats_chess) for an example.
...
create complex workflows. You can also use [`register_reply`](/website/docs/reference/agentchat/conversable_agent/#register_reply) to create new patterns.
