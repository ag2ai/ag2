...
def my_reply_function(
    ...
    messages: Optional[list[dict[str, Any]] | dict[str, Any]] = None,
...
> **Note:**
> The `messages` parameter can now be _either a list of message dicts or a single message dict_. Your reply function should handle both cases as appropriate for your logic.
...
chat_result = agent_bob.initiate_chat(
    ...
    message=[{"content":"Hi Calendar Agent!","role":"user"}],
    max_turns=2
)
