...
- `-m "not (openai or gemini)"` for skipping tests that require access to LLM services e.g. OpenAI and Gemini services.
...
```bash
bash scripts/test.sh test -m "not (openai or gemini)" --skip-docker
```

or

```bash
bash scripts/test-contrib-skip-llm.sh --skip-docker
```
