...
- `-m "not docker"`: Skips tests that explicitly require Docker. The `docker` Python package ships in the `test` dependency group, so `uv sync --group test` is enough to run these tests.
