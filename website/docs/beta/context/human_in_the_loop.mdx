...
@tool
async def execute_query(ctx: Context) -> str:
    # Pause and ask for human input
    user_response = await ctx.input(
...
To handle the input requests made by `ctx.input()`, you must register a **HITL hook** on your `Agent`. 
...
def my_hitl_hook(event: HumanInputRequest) -> HumanMessage:
    # event.content contains the prompt passed to ctx.input()
...
Alternatively, you can register or override a HITL hook using the `@agent.hitl_hook` decorator after the agent has been created.
...
!!! warning "Overriding Hooks"
    If a HITL hook is already set (for instance, via the constructor argument) and you apply the `@agent.hitl_hook` decorator, the new hook will override the existing one.
