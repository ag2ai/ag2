...
# Create a traditional user agent
...
```python linenums="1" hl_lines="14-22 32-33 44"
...
def issue_tracker(
    ...
    issue_count += 1
    ctx.variables["issue_count"] = issue_count
...
async def main():
    _, context, _ = await a_initiate_group_chat(
        pattern=pattern,
        messages="Please track this new issue.",
        max_rounds=3,
    )

    # context.data["issue_count"] will now be updated globally!
    print("Final issue count:", context.data["issue_count"])
```
