---
title: Beta NLIP Agent Integration
sidebarTitle: NLIP Agent
---

[NLIP (Natural Language Interaction Protocol)](https://ecma-international.org/publications-and-standards/standards/ecma-430/) is an ECMA-430 standard for interoperable agent communication over HTTP. The `autogen.beta.extensions.nlip` module lets you:
...
Similar to how AG2 integrates with the [A2A protocol](/docs/beta/a2a/overview), `NlipConfig` connects to a remote NLIP server over HTTP and plugs into a regular `Agent` as its LLM provider.

```python linenums="1"
from autogen.beta import Agent
from autogen.beta.extensions.nlip import NlipConfig
...
`autogen.beta` has no GroupChat — the geo lookup and the remote weather call are both tools the coordinator's LLM picks between, rather than peer chat participants:
...
```python title="coordinator.py" hl_lines="29-32 34-37"
import argparse
import asyncio
...
The coordinator connects to the NLIP weather agent through `weather_agent`, a regular `Agent` configured with `NlipConfig` (highlighted lines 29-32), and delegates to it via the plain tool function `ask_weather_agent` (highlighted lines 34-37).
