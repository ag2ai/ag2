...
Pass the strategy + trigger through [`KnowledgeConfig`](/docs/beta/advanced/agent#knowledgeconfig). The Agent wires a `_CompactionMiddleware` that fires the strategy automatically after each turn when the trigger threshold is crossed.
