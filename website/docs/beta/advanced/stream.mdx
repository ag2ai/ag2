...
By default, agent create `MemoryStream` instance internally for each conversation.

But, you can pass a custom stream to an agent when calling its `ask` method. This allows you to set up subscribers on the stream before the conversation starts, letting you observe or intercept the agent's internal events.
...
## How to subscribe on stream events
...
### Subscribe on event with specific type
...
### Subscribe on events with multiple types
...
### Subscribe on events with specific value
...
async def publish_message(ctx: Context):
    event = ModelMessage(content="Hello from the agent!")
    # Always use ctx.send() to raise events
...
async def intercept_message(
    ...
    if "secret" in event.content:
        ...
        await ctx.send(AlertEvent(message=event.content))
        return None
...
async def process_payment(ctx: Context):
    await ctx.send(PaymentProcessed(amount=100.50, status="success"))
```