...
Pass the strategy + trigger through [`KnowledgeConfig`](/docs/beta/advanced/actor#knowledgeconfig). The Actor wires a `_AggregationMiddleware` that fires `aggregate()` automatically according to the trigger.
...
If you're not using `Actor` (custom harness, tests, one-off scripts), call `#!python await strategy.aggregate(...)` yourself:
