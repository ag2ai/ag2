...
Updates `/memory/working.md` — the agent's single persistent state document. Reads the existing file, merges in context from recent events, writes the updated version back. The companion to [`WorkingMemoryPolicy`](/docs/beta/advanced/assembly#workingmemorypolicy).
...
Pass the strategy + trigger through [`KnowledgeConfig`](/docs/beta/advanced/agent#knowledgeconfig). The Agent wires a `_AggregationMiddleware` that fires `aggregate()` automatically according to the trigger.
...
Each completed aggregation emits an `AggregationCompleted` lifecycle event with `strategy` / `usage` fields for observability.
