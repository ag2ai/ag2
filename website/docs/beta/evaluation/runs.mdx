...
This page covers the core of the offline eval pipeline: building a `Suite` of tasks, calling `run_agent()`, reading the `RunResult`, and grading traces that already exist. (Saving runs and comparing them over time has its own page — [Persistence & tracking](/docs/beta/evaluation/persistence/).)
...
- **[Scorers](/docs/beta/evaluation/scorers/)** — what goes into `scorers=[...]`.
- **[Variants](/docs/beta/evaluation/variants/)** — run several builds over the suite and rank them on a leaderboard.
- **[Pairwise](/docs/beta/evaluation/pairwise/)** — judge two builds head-to-head.
- **[Persistence & tracking](/docs/beta/evaluation/persistence/)** — compare runs over time, catch regressions, and track an agent across iterations.
- **[Testing](/docs/beta/testing/)** — `TestConfig` cassettes for deterministic CI runs.
- **[Observers](/docs/beta/advanced/observers/)** — runtime safety guards (token budgets that actually halt, loop detection).
