...
Every action inside `agent.ask(...)` — model requests, model responses, tool calls, tool results — is recorded as an OpenTelemetry span. The eval framework reconstructs a `Trace` (the typed events) from those spans — the same path it uses to grade stored or cloud traces. Scorers grade what's in the trace.
...
!!! warning
    Budgets are **observational** in v0. If you need a hard kill switch, use [observers](/docs/beta/advanced/observers/) (`TokenMonitor` with `AlertPolicy(severity=FATAL)`) — the agent halts via a `HaltEvent` at runtime.
...
result.diff(baseline)              # RunDiff vs a prior run — regressions (see "Comparing runs")
...
```python linenums="1"
from autogen.beta.eval import load_run, run

baseline = load_run("runs/last_release.json")          # a previously saved run
current = await run(suite, target=my_new_agent, scorers=[...], store_dir="runs")
