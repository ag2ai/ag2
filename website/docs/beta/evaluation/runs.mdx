...
This page covers the rest of the offline eval pipeline: building a `Suite` of tasks, calling `run_agent()`, reading the `RunResult`, and the persistence format that makes runs comparable over time.
...
Just like `agent.ask(stream=...)`, both `run_agent` and `run_variants` accept a `stream`. Pass one and the runner publishes **eval lifecycle events** to it as the run unfolds — so you observe an evaluation with the same machinery you use on an agent (`subscribe`, `where`, the watch system, persistent backends). Nothing is printed for you; you attach the observer and render however you like.
...
A minimal stand-in lives in the [runnable weather example](https://github.com/ag2ai/ag2/tree/main/eval_development/runnable_weather_eval){.external-link agent="_blank"} as `compare.py` — about 80 lines, loads two JSONs and prints side-by-side deltas. Worth copying into your project if you want run-vs-run comparison today.
