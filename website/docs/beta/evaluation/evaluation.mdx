...
!!! tip
    New to evals and this moved fast? The [Get started](/docs/beta/evaluation/getting-started/) tutorial builds the same thing up one concept at a time.
...
- **[Get started](/docs/beta/evaluation/getting-started/)** — a paced, step-by-step first eval if this quick-start moved too fast.
- **[Scorers](/docs/beta/evaluation/scorers/)** — how to write them, the six prebuilts (including `agent_judge` and `failure_attribution`), return-shape rules, exception handling.
- **[Runs](/docs/beta/evaluation/runs/)** — `Suite`, the `run_agent()` signature, `RunResult` deep dive, `evaluate_traces` for existing traces, `repeats=`, and `stream=` to observe a run live.
- **[Variants](/docs/beta/evaluation/variants/)** & **[Pairwise](/docs/beta/evaluation/pairwise/)** — compare builds on a leaderboard, or head-to-head.
- **[Persistence & tracking](/docs/beta/evaluation/persistence/)** — the run JSON, comparing runs for regressions, and tracking an agent across iterations.
