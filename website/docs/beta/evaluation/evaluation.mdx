...
4. A call to **run** — the framework's entry point.
...
from autogen.beta.eval import run, scorer
...
!!! tip
    Building an actual eval? See the [runnable weather example](https://github.com/ag2ai/ag2/tree/main/eval_development/runnable_weather_eval){.external-link target="_blank"} — five tasks, eight scorers (five prebuilt + three custom), deterministic mode for CI plus a live-model mode.
...
Every `run()` returns a `RunResult` and writes a JSON file to `store_dir`. The result exposes:
...
- **[Runs](/docs/beta/evaluation/runs/)** — `Suite`, the `run()` signature, `RunResult` deep dive, the JSON schema, budget thresholds. Also: **`repeats=`** for consistency, **`run_variants` + `Variants`** to compare models / prompts / tools and rank them, and **`stream=`** to observe a run live via its event stream.
