...
| `outputs` | `#!python dict[str, Any]` | The agent's final answer, projected from the trace — `{"body": <final model-response text>}`. |
...
Four are shipped under `autogen.beta.eval.scorers` — frozen versions of the most common patterns.
