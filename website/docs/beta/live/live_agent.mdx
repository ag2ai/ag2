...
`LiveAgent` is a full-duplex voice agent backed by a provider's realtime API. Unlike the [turn-by-turn STT/TTS pipeline](/docs/beta/live/stt_tts), it opens a single bidirectional session for the entire conversation — audio flows in and out continuously, with built-in voice activity detection and barge-in.
...
- **[STT & TTS](/docs/beta/live/stt_tts)** — the lower-latency turn-by-turn alternative.
- **[Tools](/docs/beta/tools/tools)** — tool authoring, middleware, and approval flows that all work inside a `LiveAgent`.
