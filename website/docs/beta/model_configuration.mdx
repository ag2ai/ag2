...
`GeminiConfig` supports both the public **Gemini Developer API** (via `api_key`) and **Vertex AI** on Google Cloud.

To use Vertex AI, set `vertexai=True` and supply your GCP `project` and `location`. Authentication then accepts any of the following:
...
| Environment variable | Equivalent parameter | Notes |
| :--- | :--- | :--- |
| `GOOGLE_API_KEY` | `api_key` | Developer API (non-Vertex). Takes precedence over `GEMINI_API_KEY` if both are set. |
| `GEMINI_API_KEY` | `api_key` | Developer API (non-Vertex). |
| `GOOGLE_GENAI_USE_VERTEXAI` | `vertexai` | Set to `true` / `1` to enable Vertex mode without passing `vertexai=True`. |
| `GOOGLE_CLOUD_PROJECT` | `project` | GCP project ID for Vertex. |
| `GOOGLE_CLOUD_LOCATION` | `location` | GCP region (or `global`) for Vertex. |
| `GOOGLE_APPLICATION_CREDENTIALS` | `credentials` | Path to a service-account JSON key file, read via ADC. |

With all four Vertex variables set in the environment, configuration collapses to just the model name:
