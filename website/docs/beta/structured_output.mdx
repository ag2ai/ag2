...
If the model’s output cannot be parsed or fails validation, `validate()` raises an error from the underlying parser (for example Pydantic’s validation errors). You can pass `retry` to automatically [re-ask the model](#validation-retries) on failure.
...
When the model's response fails schema validation, you can automatically **re-ask** the model instead of raising immediately. Pass the `retry` keyword to `validate()`:

```python linenums="1" hl_lines="4"
agent = Agent("assistant", config=config, response_schema=int)

reply = await agent.ask("How many planets in the solar system?")
result = await reply.validate(retry=3)
```

The `retry` parameter controls how many **total attempts** are made (including the first). With `retry=3`, the initial response is validated; if it fails, the model is re-asked up to **2 more times** before the error is raised.

| Value | Behavior |
|-------|----------|
| `retry=False` (default) | No retries — raise on the first validation failure. |
| `retry=3` | Up to 3 total attempts (1 initial + 2 re-asks). |
| `retry=True` | Re-ask indefinitely until the model produces a valid response. |
...
!!! warning
    `retry=True` will loop forever if the model consistently produces invalid output. Use a finite count in production, and reserve `True` for interactive or experimental use.
...
Instead of asking the model to produce a bare value like `42`, the API schema asks for `{"data": 42}`. The `validate()` method transparently unwraps the envelope so your code still receives a plain Python value.
...
# With embed=True (default): model produces {"data": 42}, validate() returns 42 either way
```
...
!!! note
    Embedding is transparent to your code. Whether `embed` is `True` or `False`, `validate()` always returns the unwrapped Python value. The only difference is the JSON shape the model is asked to produce.
