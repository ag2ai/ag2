...
The most straightforward way to create a tool is by using the `@tool` decorator. This decorator automatically parses your function's signature, type hints, and docstring to generate a schema that the LLM can understand.
...
Alternatively, you can register a tool directly to an existing agent instance using its `@agent.tool` decorator. 

This approach is particularly useful when you need to dynamically add capabilities to an agent after it has been created, or when you are organizing your code by attaching specific tools to specific agent instances logically.
...
LLMs perform best when they have precise constraints and detailed instructions. While the framework automatically generates a tool's schema based on its function signature and docstring, you might want to provide more granular control—such as defining minimum/maximum values, enforcing specific formats, or overriding the tool's name.
...
def set_temperature(
    temp: Annotated[
        int,
        Field(..., description="The target temperature.", ge=10, le=30)
    ],
    mode: Annotated[
        str,
        Field(..., description="The thermostat mode.", pattern="^(heat|cool|auto)$")
    ]
) -> str:
