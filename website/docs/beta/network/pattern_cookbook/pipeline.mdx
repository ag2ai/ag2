...
**Classic (non-beta) primitives:** `#!python DefaultPattern` with explicit
...
async def main() -> None:
    ...
    validator = await hub.register(validator_agent, Passport(name="validator"), Resume())
    enricher = await hub.register(enricher_agent, Passport(name="enricher"), Resume())
    fulfilment = await hub.register(fulfilment_agent, Passport(name="fulfilment"), Resume())
