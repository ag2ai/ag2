...
| Confidence threshold via `#!python ExpressionContextCondition("confidence < 7")` | Custom `#!python ContextThreshold` condition (recipe in [Context Variables → Custom Conditions](/docs/beta/network/context_variables#custom-conditions)) |
...
async def main() -> None:
    ...
    user = await hub_obj.register(user_agent, Passport(name="user"), Resume())
    tier1 = await hub_obj.register(tier1_agent, Passport(name="tier1"), Resume())
    tier2 = await hub_obj.register(tier2_agent, Passport(name="tier2"), Resume())
    senior = await hub_obj.register(senior_agent, Passport(name="senior"), Resume())
