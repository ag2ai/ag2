...
async def main() -> None:
    ...
    router = await hub_obj.register(router_agent, Passport(name="router"), Resume())
    billing = await hub_obj.register(billing_agent, Passport(name="billing"), Resume())
    technical = await hub_obj.register(technical_agent, Passport(name="technical"), Resume())
    general = await hub_obj.register(general_agent, Passport(name="general"), Resume())
