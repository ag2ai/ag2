...
* **No `finish_delegate` tool.** Earlier iterations of this demo had a
  ...
  are safe because [first-emitted-wins](/docs/beta/network/workflow#first-emitted-wins)
...
async def main() -> None:
    ...
    intake = await hub.register(intake_agent, Passport(name="intake"), Resume())
    coord = await hub.register(coord_agent, Passport(name="coord"), Resume())
    researcher = await hub.register(researcher_agent, Passport(name="researcher"), Resume())
    writer = await hub.register(writer_agent, Passport(name="writer"), Resume())
