...
**Classic (non-beta) primitives:** `#!python DefaultPattern` with parallel dispatch,
...
async def main() -> None:
    ...
    intake = await hub_obj.register(intake_agent, Passport(name="intake"), Resume())
    safe = await hub_obj.register(safe_agent, Passport(name="safe"), Resume())
    clever = await hub_obj.register(clever_agent, Passport(name="clever"), Resume())
    nerdy = await hub_obj.register(nerdy_agent, Passport(name="nerdy"), Resume())
    evaluator = await hub_obj.register(evaluator_agent, Passport(name="evaluator"), Resume())
