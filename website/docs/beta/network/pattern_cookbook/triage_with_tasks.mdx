...
async def main() -> None:
    ...
    triage = await hub_obj.register(triage_agent, Passport(name="triage"), Resume())
    researcher = await hub_obj.register(researcher_agent, Passport(name="researcher"), Resume())
    writer = await hub_obj.register(writer_agent, Passport(name="writer"), Resume())
    reviewer = await hub_obj.register(reviewer_agent, Passport(name="reviewer"), Resume())
