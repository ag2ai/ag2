...
!!! warning "Why the routing tools must be terminal (`final=True`)"
    ...
    Returning `#!python ToolResult(..., final=True)` makes each routing tool **terminal**: the first call ends the round, so every coordinator turn carries exactly one routing intent. Pair it with disabling parallel tool calls on the coordinator's config (`#!python disable_parallel_tool_use` — see the [Star pattern](/docs/beta/network/pattern_cookbook/star)'s "Parallel-call defence" note) so the model also commits to one tool per response.
...
async def main() -> None:
    ...
    user = await hub_obj.register(user_agent, Passport(name="user"), Resume(), attach_plugin=False)
    ...
    critic = await hub_obj.register(critic_agent, Passport(name="critic"), critic_resume, attach_plugin=False)
...
* If specialists hand off to each other (specialist → specialist without going through the coordinator), you need explicit `#!python FromSpeaker → AgentTarget` rules — the default-to-coordinator target no longer covers it. See [Hierarchical](/docs/beta/network/pattern_cookbook/hierarchical) for nested coordination.
* If the LLM needs to decide *whether* to continue rather than *who* to ask, you may want a `#!python ContextEquals` gate. See [Feedback Loop](/docs/beta/network/pattern_cookbook/feedback_loop).
* If you need parallel fan-out + synthesis (ask all specialists, then summarise), see [Star](/docs/beta/network/pattern_cookbook/star) — same idea, different control flow.

## See Also

- [Workflow Adapter](/docs/beta/network/workflow) — the underlying graph machinery, dynamic `#!python Handoff`, and `#!python Finish` typed return.
- [Star](/docs/beta/network/pattern_cookbook/star) — fan-out variant with WAL-gated synthesis instead of LLM-driven turn-by-turn selection.
- [Hierarchical](/docs/beta/network/pattern_cookbook/hierarchical) — when specialists need their own sub-coordinators.
- [Migrating from Group Chat](/docs/beta/network/migration_from_group_chat) — the broader translation table for classic patterns.
