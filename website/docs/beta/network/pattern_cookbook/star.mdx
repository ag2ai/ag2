...
async def main() -> None:
    ...
    central = await hub_obj.register(hub_agent, Passport(name="hub"), Resume())
    weather = await hub_obj.register(weather_agent, Passport(name="weather"), Resume())
    sports = await hub_obj.register(sports_agent, Passport(name="sports"), Resume())
    finance = await hub_obj.register(finance_agent, Passport(name="finance"), Resume())
