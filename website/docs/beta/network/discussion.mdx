...
`#!python discussion` is an N-party round-robin session. Participants speak in a fixed order, cycling indefinitely until you close it. The adapter enforces "wait your turn" via `#!python validate_send`; the hub's `#!python can_send` probe lets the default handler skip wasted LLM calls when it isn't this agent's turn.
...
| Auto-close | No |
| Termination | Explicit `#!python session.close()` or TTL |
...
To halt, cap on text count and call `#!python session.close()`:
...
- **bob's handler** — calls `#!python hc.can_send(session_id, bob.agent_id)`. The adapter says "yes, bob is `expected_next_speaker`." Handler runs `#!python Agent.ask`, sends bob's reply.
- **carol's handler** — calls `#!python hc.can_send(session_id, carol.agent_id)`. The adapter says "no, expected_next_speaker is bob, not carol." Handler returns without engaging the LLM.
...
- Conditional handoffs ("if alice mentions security, hand to the security expert") — use [`#!python workflow`](/docs/beta/network/workflow).
- Two participants only with no order — use [`#!python conversation`](/docs/beta/network/conversation).
- A pipeline where each step happens once — use [`#!python workflow`](/docs/beta/network/workflow) with `#!python TransitionGraph.sequence(...)`.
...
Protocol envelopes (`#!python EV_SESSION_*`, `#!python ag2.task.*`) bypass the turn check.
...
Read via `#!python hub._adapter_states[session_id]`. The order is fixed at create time by sorting participants on `#!python Participant.order`; round-robin advances by `(current_index + 1) % len(participant_order)`.
...
`#!python discussion` never auto-closes. The example below caps at 6 turns and calls `#!python session.close()`, but four other patterns work for `#!python discussion` too:

* **App-side cap** — count turns and call `#!python session.close()` (canonical, simplest).
* **Agent-side tool** — any participant calls a tool that closes the session. See `#!python ms_code/network/03a_discussion_agent_close.py`.
...
See [Closing Sessions](/docs/beta/network/termination) for the full picture.

## Working Examples

* `ms_code/network/03_discussion_3way.py` — three real Sonnet agents in character, six turns, then explicit close.
* `ms_code/network/03a_discussion_agent_close.py` — same three agents but each has an `#!python end_conversation` tool; the LLM decides when to wrap up.
