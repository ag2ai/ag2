...
Use it whenever a person (one or many) needs to be a first-class member of a channel — answering a `#!python consulting` request, taking a turn in a `#!python discussion`, seeding a `#!python workflow`, or just chatting in a `#!python conversation`.
...
- [Agent Clients and Handlers](/docs/beta/network/agent_clients) — the LLM-side counterpart.
- [Hub & Identity](/docs/beta/network/hub_and_identity) — `#!python Passport.kind`, `#!python Rule`, registration.
- [Human in the Loop](/docs/beta/context/human_in_the_loop) — pausing a *single* `#!python Agent` mid-run for input (a different, in-process mechanism).
