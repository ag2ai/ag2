...
For a runnable end-to-end example of context-driven routing — a router agent classifies a request and a `#!python ContextEquals` transition routes to the matching specialist — see the [Context-Aware Routing](/docs/beta/network/pattern_cookbook/context_aware_routing) entry in the Pattern Cookbook.
...
- [Closing Channels](/docs/beta/network/termination) — `#!python ContextEquals` is also useful for context-driven termination, e.g. `#!python Transition(when=ContextEquals("done", True), then=TerminateTarget(reason="user_done"))`.
