...
Session-scoped mutable state that any participant can read or write,
...
`#!python autogen.agentchat.group`, scoped to one workflow session.

## The Mechanism

Context variables live on `#!python WorkflowState.context_vars: dict[str, Any]`
— a field folded under the per-session WAL lock. There's no parallel
...
1. **Mutation** — anyone in the session emits an `#!python EV_CONTEXT_SET`
...
2. **Read** — transition conditions get `#!python state` as their first arg, so
   ...
   directly. Tools can also inject `#!python SessionStateInject` to read.
...
Any participant of the session can write `#!python EV_CONTEXT_SET`, regardless
of whose turn it is. The per-session WAL lock serialises concurrent
...
Sender must be a participant of the session — non-participants are
rejected by `#!python validate_send`.

## Setting Values

Send the envelope from any participant via the existing `#!python Session.send`:

```python linenums="1" hl_lines="9 10 11 12 13 14"
from autogen.beta.network import EV_CONTEXT_SET, SessionInject


async def set_route(route: str, session: SessionInject) -> str:
    """Record the routing decision for this session."""
    if session is None:
...
```python linenums="1"
from autogen.beta.network import SessionStateInject


async def increment_counter(session: SessionInject, state: SessionStateInject) -> str:
...
Pre-populate context at session creation by passing a `#!python context_vars`
...
session's adapter state is reconstructed by replaying the WAL through
...
`#!python ms_code/network/03b_workflow_context_vars.py` — a triage workflow
where a moderator agent calls a `#!python set_route` tool to write
`#!python {"route": "security"}` or `#!python {"route": "legal"}` to context, and a
`#!python ContextEquals` transition routes to the matching specialist agent.
End-to-end against real Sonnet:

```bash
python ms_code/network/03b_workflow_context_vars.py
```
...
|---|---|---|
| Mutable session-scoped dict | `#!python ContextVariables(...)` passed to `#!python initiate_chat` | `#!python WorkflowState.context_vars` |
...
| Auto-render into LLM prompt | Built-in | Not yet — write a middleware that reads `#!python SessionStateInject.context_vars` and prepends to the prompt |
...
- [Workflow Adapter](/docs/beta/network/workflow) — graphs, transitions, targets, conditions, and a "Context-Driven Transitions" section dedicated to routing patterns.
- [Closing Sessions](/docs/beta/network/termination) — `#!python ContextEquals` is also useful for context-driven termination, e.g. `#!python Transition(when=ContextEquals("done", True), then=TerminateTarget(reason="user_done"))`.
- [Pattern Cookbook](/docs/beta/network/pattern_cookbook/pattern_cookbook) — eight canonical orchestrations (Pipeline, Star, Feedback Loop, Triage-with-Tasks, etc.) translated from classic AG2.
- [Migrating from Group Chat](/docs/beta/network/migration_from_group_chat) — side-by-side translation of classic patterns.
