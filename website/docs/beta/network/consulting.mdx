...
`#!python consulting` is a strict 1-question-1-reply session. The initiator sends exactly one substantive envelope; the respondent sends exactly one reply; the adapter auto-closes with reason `"consulting_complete"`.
...
1. `#!python alice.open(type="consulting", target="bob")` — hub posts invite to bob; bob auto-acks; session goes `ACTIVE`.
2. `#!python session.send(...)` — alice's first (and only) envelope.
...
5. Hub posts `#!python EV_SESSION_CLOSED`. `#!python alice.wait_for_session_event(...)` wakes.
...
- Multi-turn back-and-forth — use [`#!python conversation`](/docs/beta/network/conversation).
- Multiple respondents — use [`#!python discussion`](/docs/beta/network/discussion) or [`#!python workflow`](/docs/beta/network/workflow).
...
`#!python validate_send` raises `#!python ProtocolError`; the hub propagates it back to the sender's `#!python session.send(...)` call.
...
| Explicit `#!python session.close()` | `"explicit_close"` (or whatever string you pass) |
...
The reason flows on the `#!python EV_SESSION_CLOSED` envelope's `event_data` and is also stored in `#!python SessionMetadata.close_reason` for later inspection via `#!python hub.get_session(...)`.

For the full set of close patterns (agent-side tool, sentinel, TTL safety nets), see [Closing Sessions](/docs/beta/network/termination).

## Working Example

`ms_code/network/01_hello_network.py` is the canonical consulting example with real Sonnet on both sides — the same shape as the snippet above, with audit/WAL inspection.
