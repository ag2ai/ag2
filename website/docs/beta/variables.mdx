...
def fetch_user_data(
    ...
    return f"Fetching {user_id} using {db}"
```

!!! note
    Improtant remark: all such annotations has no effect on the tool schema for the LLM.
    It's just a technical detail for the framework to inject the value into the function.
...
Variables are mutable. If a tool updates a `ctx.variables` dictionary, that change is preserved in the context and will be available to subsequent tool calls within the same conversation.
...
@tool
def authenticate(ctx: Context) -> str:
    # Generate and store a token in the context variables
    ctx.variables["auth_token"] = "abc-123"
...
def fetch_secure_data(
    ...
    return f"Data fetched with token {token}"
```