...
```python linenums="1"
from autogen.beta import Agent
from autogen.beta.middleware import SamplingMiddleware, TelemetryMiddleware
...
agent = Agent(
    ...
    middleware=[
        SamplingMiddleware(
            rate=0.1,
            middleware=TelemetryMiddleware(tracer_provider),
        )
    ],
)
```

Wraps another middleware and activates it on a random fraction of agent turns.
One sampling decision is made per turn — all hooks within a sampled turn share the same on/off state, keeping traces coherent.
A tool execution that happens in a sampled turn is always observed; no partial traces.

`rate` is a float in `[0.0, 1.0]`:
- `1.0` — inner middleware always active (equivalent to using the middleware directly).
- `0.0` — inner middleware never active (useful for temporarily disabling tracing without removing it from the stack).
- Values in between — the inner middleware runs on approximately that fraction of turns.

Use `SamplingMiddleware` when observability overhead is a concern and representative sampling is acceptable — for example, tracing 10 % of turns in a high-traffic deployment while keeping the other 90 % at full speed.
