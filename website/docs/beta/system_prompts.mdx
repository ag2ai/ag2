...
System prompts can be generated dynamically when a conversation starts. This is useful when the prompt depends on external state or initial context. You can achieve this by using the `@agent.prompt` decorator or passing an asynchronous function.
...
conversation = await agent.ask("Hi!", prompt=["Temporary prompt for this run"])
```

## Prompt updates

For continuous and event-driven prompt updates, you can mutate `ctx.prompt` dynamically from an event subscriber. This allows you to respond to specific events in the stream and adjust the agent's behavior on the fly during an ongoing conversation.
