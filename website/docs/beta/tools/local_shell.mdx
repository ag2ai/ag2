...
`SandboxShellTool` lets an agent run shell commands inside an **environment** you choose — a local subprocess, a Docker container, a Daytona sandbox, or any custom backend. Unlike the provider-native [`ShellTool`](/docs/beta/tools/builtin_tools#shell) (which runs server-side and only on Anthropic/OpenAI), it executes client-side, so it works with **any model provider**.
...
The same environment can back both a `SandboxShellTool` and a [`SandboxCodeTool`](/docs/beta/tools/code_execution).
...
| :--- | :--- | :--- |
| `environment` | `None` | Backend (`LocalEnvironment` / `DockerEnvironment` / `DaytonaEnvironment`), bare `Sandbox`, or a pre-built `ShellAdapter`. `None` → `LocalEnvironment()` |
