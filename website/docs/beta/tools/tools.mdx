...
To run async hooks around **one** tool (argument normalization, result redaction, bundled auditing), pass `middleware=[...]` with `ToolMiddleware` callables. See the dedicated [Tool middleware](/docs/beta/tools/tool_middleware) page.
