...
Tool middleware lets you wrap **one** function tool with async hooks that run immediately around its execution—same idea as agent `on_tool_execution()` in [Middleware](/docs/beta/middleware), but attached at **tool definition** time with plain callables (no `BaseMiddleware` subclass).
...
The public type alias is **`ToolMiddlewareHook`** in `autogen.beta.middleware`.
