...
`PerplexitySearchTool` gives an agent web search powered by [Perplexity](https://www.perplexity.ai){.external-link target="_blank"}'s Sonar models. Each call returns an LLM-generated answer alongside the search results, citations, and optional images used to ground it.
...
from autogen.beta import Agent
from autogen.beta.config import AnthropicConfig
from autogen.beta.tools import PerplexitySearchTool

agent = Agent(
    "researcher",
    config=AnthropicConfig(model="claude-sonnet-4-6"),
    tools=[PerplexitySearchTool(api_key=os.environ["PERPLEXITY_API_KEY"])],
)
```
...
The tool returns a `SearchResponse` with these fields:

| Field | Description |
| :--- | :--- |
| `query` | The original search query |
| `content` | LLM-generated answer grounded in the sources |
| `search_results` | List of `SearchResult` (`title`, `url`, `date`, `snippet`) used by the model |
| `citations` | URLs the model cited inline |
| `images` | List of `ImageMeta` returned when `return_images=True` |
...
!!! note
    `PerplexitySearchTool` calls Perplexity's [Sonar API](https://docs.perplexity.ai/docs/sonar/openai-compatibility){.external-link target="_blank"} — an LLM-grounded answer with citations, not raw search hits. For unprocessed ranked search results, use Perplexity's separate Search API.

!!! tip
    `search_domain_filter` is a Pro-tier feature on the Perplexity API; see [usage tiers](https://docs.perplexity.ai/guides/usage-tiers){.external-link target="_blank"}.

---

## PerplexitySearchAPITool

`PerplexitySearchAPITool` gives an agent web search via Perplexity's dedicated [Search API](https://docs.perplexity.ai/docs/search/quickstart){.external-link target="_blank"}. It returns ranked results (title, URL, snippet, date) without generating an LLM answer, which keeps token usage minimal.

!!! note
    Requires the `perplexity` extra and an API key: `pip install ag2[perplexity]`. The tool reads `PERPLEXITY_API_KEY` automatically when `api_key` is omitted.

```python linenums="1"
import os
from autogen.beta import Agent
from autogen.beta.config import AnthropicConfig
from autogen.beta.tools import PerplexitySearchAPITool

agent = Agent(
    "researcher",
    config=AnthropicConfig(model="claude-sonnet-4-6"),
    tools=[PerplexitySearchAPITool(api_key=os.environ["PERPLEXITY_API_KEY"])],
)
```
...
```python linenums="1"
tool = PerplexitySearchAPITool(
    max_results=10,
    search_domain_filter=["arxiv.org", "-medium.com"],   # prefix '-' to exclude
    search_recency_filter="month",                        # "hour"|"day"|"week"|"month"|"year"
    search_after_date_filter="1/1/2025",                  # m/d/yyyy
    search_before_date_filter="12/31/2025",
)
```

Domain filters are either an allowlist or a denylist — do not mix the two; use a leading `-` to negate. All parameters accept a `Variable` for dynamic values resolved at execution time.
