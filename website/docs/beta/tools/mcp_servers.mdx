...
`MCPServer` is a [Toolkit](/docs/beta/tools/toolkits) — it discovers the server's tools lazily and exposes each one as a regular function tool to the agent. It accepts either a remote URL/`MCPServerConfig` or an `MCPStdioServerConfig` for a locally-launched subprocess; the rest of the agent doesn't care which transport is in use.
...
`MCPStdioServerConfig` accepts `command`, `args`, `env`, `cwd`, `encoding`, `server_label`, `description`, `allowed_tools`, and `blocked_tools`. Any of these can be a `Variable` if the value is only known at runtime — handy for injecting per-conversation tokens or workspace paths.
