...
`FilesystemToolset` accepts a `middleware=[...]` parameter to wrap all tools with shared hooks. This works the same as [Toolkit middleware](/docs/beta/tools/toolkits#toolkit-middleware):
...
fs = FilesystemToolset(base_path="/tmp/workspace", middleware=[confirm_writes])
```
