...
!!! tip
    To trace the interactions *between* agents in a network — the hub's dispatch, channels, agent lifecycles, and tasks — see [Network Telemetry](/docs/beta/telemetry/network). The two compose: an agent's `#!python invoke_agent` span links back to the network envelope that triggered it.
