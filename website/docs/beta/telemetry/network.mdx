...
Where [Agent Telemetry](/docs/beta/telemetry/agent) traces what happens *inside* one agent, **network telemetry** traces what happens *between* them — the hub's envelope dispatch, channel lifecycles, agent registration, and tasks. It is emitted by the hub itself, as [OpenTelemetry](https://opentelemetry.io/){.external-link target="_blank"} spans on the same `#!python TracerProvider` your agents use, so the two stitch together into one picture.

Like agent telemetry it is **opt-in**: nothing is traced unless you hand the hub a `#!python TracerProvider`.

!!! tip
    Network telemetry composes with [Agent Telemetry](/docs/beta/telemetry/agent). When an agent's turn is triggered by a network envelope, its `#!python invoke_agent` span links back to that envelope's span — so a trace can follow a message from dispatch, into the receiving agent's LLM calls and tools, and back out.
...
```python linenums="1" hl_lines="31 35 44"
import asyncio
...
Agents instrumented with `#!python TelemetryMiddleware` add their own `agent` / `llm` / `tool` / `human_input` spans (see [Agent Telemetry](/docs/beta/telemetry/agent)), linked to the triggering envelope.

!!! note
    `#!python network.task` spans cover capability-tagged tasks — the same `#!python agent.task(..., capability="X")` mechanism described in [Task Observation](/docs/beta/network/task_observation). The span records the task's `capability`, `outcome`, and latency, which is what feeds task-level evaluation views.
...
| `ag2.network.event_type` | envelope | Envelope event type (e.g. `ag2.text`) |
...
| `ag2.network.outcome` | task | Terminal outcome (e.g. `completed`, `expired`) |
...
Channel spans also record diagnostic span **events** — `dispatch_failed`, `turn_failed`, `envelope_rejected`, and expectation violations — and `agent.lifetime` spans record `inbox_pressure`, so failures and back-pressure surface in the trace without their own spans.
