...
- **Expectations** — per-channel SLAs (`reply_within`, `max_silence`, `turn_within`) with explicit violation handlers (`auto_close`, `warn`, `audit`).
...
- **Deeper TransitionGraph rules** — `FromSpeaker`, `ContextEquals`, `ToolCalled`, and dynamic `Handoff` and 'Finish' returns let a workflow react to *what* was said, not just *who* said it.

The next four sections walk through each knob and end with a single centerpiece that combines all three. Every knob is a first-class envelope on the channel's write-ahead log — so the choreography you dial in survives a fold replay, the same way the conversation does.
...
**Example 1: `reply_within` auto-close on a consulting channel.** Alice asks Bob a question. Bob never replies. The hub closes the channel after the threshold.
...
**Example 2: `max_silence` warn on a conversation channel.** Alice posts once, then both sides go quiet. The `warn` handler fires and broadcasts `ag2.expectation.violated` — the channel stays open so the participants can choose what to do with the signal.
