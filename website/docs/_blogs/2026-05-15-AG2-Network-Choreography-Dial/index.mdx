...
In parallel, to build strong and capable agents, we're preparing a companion post, **The Agent Harness: An Agent Is More Than a Loop**, zooms *inside* a single agent to make them long-running and knowledge-centric. Keep an eye out for that one (we'll update this when it's ready).
...
But the starting notch isn't the only knob. Three more sit on top of every adapter, and you reach for them when "what conversation shape do I want?" turns into "what should happen when the room goes quiet, when two participants need a side bar, or when the next speaker depends on what the previous one returned?":

- **Expectations** — per-channel SLAs (`reply_within`, `max_silence`, `turn_within`) with explicit violation handlers (`auto_close`, `warn`, `audit`). A timer that does something is choreography with teeth.
...
The next four sections walk through each knob and end with a single centerpiece that combines all three. Every knob is a first-class envelope on the channel's write-ahead log — so the choreography you dial in survives a fold replay byte-for-byte, the same way the conversation does.

## Expectations and violation handlers

![A channel timeline with reply_within(30s, auto_close) armed: the countdown timer ticks down, no reply lands, the handler fires, an EV_CHANNEL_CLOSED envelope appears on the WAL.](img/n2-expectations.webp)

Adapters ship with default expectations — `consulting` arms a `reply_within(600s, auto_close)` out of the box so channels don't leak when a respondent goes dark. You override them by passing an `expectations` list in the channel's `knobs` when you open it. Each entry has a `name`, an `on_violation` handler, and a `params` dict with the threshold.
...
| `turn_within` | the expected next speaker hasn't posted within `seconds` |
...
| `warn` | broadcast `ag2.expectation.violated` to every participant and stay open |
| `audit` | record to the audit log and do nothing visible |

The hub evaluates expectations on a periodic sweep (10 s by default). All three knobs are pure data on the WAL — the sweep is stateless, so a hub restart re-derives the same state and the same timers pick up exactly where they left off.

**Example: `reply_within` auto-close on a consulting channel.** Alice asks bob a question. Bob never replies. The hub closes the channel after the threshold.
...
async def main() -> None:
    ...
    # Override the default consulting expectation with a short reply_within.
    ...
    await channel.send("bob, can you review this before the meeting?")

    close_env = await alice.next_envelope(
        predicate=lambda e: (
    ...
    # → "expectation_violated:reply_within"
...
**Example: `max_silence` warn on a conversation channel.** Alice posts once, then both sides go quiet. The `warn` handler fires and broadcasts `ag2.expectation.violated` — the channel stays open so the participants can choose what to do with the signal.
...
async def main() -> None:
    ...
    await channel.send("hey bob, what do you think about this design?")

    # Both sides go quiet. warn fires, alice sees the violation envelope.
    viol_env = await alice.next_envelope(
        predicate=lambda e: (
            e.channel_id == channel.channel_id
            and e.event_type == EV_EXPECTATION_VIOLATED
        ),
        timeout=120.0,
    )
    print(f"violation: {viol_env.event_data}")
    # → {'expectation': 'max_silence', 'violators': [], 'detail': {...}}
    # Channel is still OPEN — warn never closes it.
...
The two examples cover opposite ends of the spectrum: `auto_close` is the "tear it down" path for production workflows where a stalled step is a failure; `warn` is the "signal but stay" path for human-in-the-loop flows where you want observability without force.
...
Private side-channels use the same primitive: open a second channel with a restricted participant list. The two channels run on separate WALs that never intersect. An agent on the main channel cannot even discover that the side-channel exists, let alone read its messages.

The example below has three participants on a discussion channel. After a public exchange, alice opens a private `consulting` channel with carol — bob is not invited and never sees a single byte of that conversation.
...
When `classify()` returns `Handoff(target=specialist_id)`, the fold reads `routing.target` directly and sets `expected_next_speaker = specialist_id`, bypassing `select_next` entirely. The graph edge is never consulted. The `FromSpeaker(drafter)` fallback only fires if the drafter writes a plain reply without calling `classify`.

**ContextEquals — routing on state, not sender.** `ContextEquals(key, value)` fires when `channel.context_vars[key] == value`. You write context vars from a `@tool` via `set_context(channel, key, value)`, which emits an `EV_CONTEXT_SET` envelope. The fold applies the mutation before evaluating transitions, so the same turn that sets a var can immediately route on it:
...
## Centerpiece runnable — all three knobs at once

![A workflow channel with four roles (user / drafter / specialist / reviewer). A dynamic Handoff arc jumps from drafter directly to specialist; specialist sets a context var; a ContextEquals rule routes to reviewer; a turn_within countdown ticks above reviewer's card and fires when reviewer doesn't speak, dropping an EV_CHANNEL_CLOSED · turn_within envelope onto the WAL.](img/n2-centerpiece.webp)

The final example combines all three knobs on a single workflow channel:
...
2. **ContextEquals routing** — the specialist calls `flag(status='needs_review')` which writes `status` to the channel's `context_vars`. A `ContextEquals("status", "needs_review")` transition fires on the specialist's round-end packet and routes to the reviewer.
3. **`turn_within` auto-close** — the reviewer is a `HumanClient` that never responds. The expectation fires after 90 seconds and closes the channel with reason `expectation_violated:turn_within`.
...
from autogen.beta.knowledge import MemoryKnowledgeStore
from autogen.beta.network import (
    EV_CHANNEL_CLOSED, EV_CONTEXT_SET, EV_PACKET, EV_TEXT,
...
async def main() -> None:
    ...
    user_hc, drafter_hc, spec_hc, rev_hc = (HubClient(link, hub=hub) for _ in range(4))
    ...
    specialist_agent = Agent(
        "specialist",
        prompt="Call flag(status='needs_review') first, then write your 2-sentence analysis.",
        config=config,
    )
    ...
    reviewer = await rev_hc.register_human(Passport(name="reviewer", kind="human"))
    ...
    graph = TransitionGraph(
        ...
        transitions=[
            ...
            # ContextEquals fires first (priority -1); ToolCalled backup forces EV_PACKET.
            Transition(
                when=ContextEquals("status", "needs_review"),
                then=AgentTarget(reviewer.agent_id),
                priority=-1,
            ),
            Transition(when=ToolCalled("flag"), then=AgentTarget(reviewer.agent_id)),
            Transition(when=FromSpeaker(specialist.agent_id), then=AgentTarget(reviewer.agent_id)),
        ],
        default_target=TerminateTarget("done"),
        max_turns=12,
    )
    ...
    channel = await user.open(
        type="workflow",
        target=[drafter.agent_id, specialist.agent_id, reviewer.agent_id],
        knobs={
            "graph": graph.to_dict(),
            "expectations": [
                {"name": "turn_within", "on_violation": "auto_close", "params": {"seconds": 90}},
            ],
        },
    )
    await channel.send(
    ...
    print(f"closed: {close_env.event_data.get('reason')!r}")
    # → 'expectation_violated:turn_within'

    for hc in (user_hc, drafter_hc, spec_hc, rev_hc):
        await hc.close()
    await hub.close()
...
```
        user  ag2.channel.invite          (× 3 — drafter, specialist, reviewer)
     drafter  ag2.channel.invite.ack
  specialist  ag2.channel.invite.ack
    reviewer  ag2.channel.invite.ack
  ...
  specialist  EV_CONTEXT_SET : {'status': 'needs_review'}
  specialist  EV_PACKET : routing=handoff  body=## Security Implications of DoH…
        user  ag2.channel.closed          reason=expectation_violated:turn_within
```

Every knob lands exactly once, in the right place, in the right order. The `EV_CONTEXT_SET` is the specialist writing to the channel's state; the `EV_PACKET` that follows is the turn-end envelope whose fold evaluates `ContextEquals("status", "needs_review")` and advances to the reviewer. The `EV_CHANNEL_CLOSED` at the bottom is the expectation doing its job — the reviewer never replied.

## Wrapping up

Three dials, one primitive. Expectations govern *time* — how long any step can stall before the hub acts. Audience addressing governs *visibility* — who sees which envelope on which thread. `TransitionGraph` conditions govern *routing* — who speaks next based on what was said or what state was written. Each is a first-class envelope on the WAL, which means each one survives a hub restart exactly as written.
