...
![AG2 OpenTelemetry Tracing](img/robot_telemetry.jpg)
...
AG2's tracing follows the [OpenTelemetry GenAI Semantic Conventions](https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-agent-spans/) so your agent traces are structured the same way as traces from other GenAI tools and frameworks.
...
Instruments **all** LLM calls globally by patching `OpenAIWrapper.create()`. Each call produces a `chat {model}` span with provider, model, token usage, cost, and request parameters.
...
flowchart TD
    subgraph Client Process
        C1["conversation user_proxy"]
        C2["invoke_agent triage_agent"]
        C3["chat gpt-4o-mini"]
        C4["invoke_agent tech_agent\n(gen_ai.agent.remote = true)"]
        C1 --> C2 --> C3
        C1 --> C4
    end
    subgraph Server Process — tech_agent A2A server
        S1["a2a-execution"]
        S2["conversation tech_agent"]
        S3["invoke_agent tech_agent"]
        S4["chat gpt-4o-mini"]
        S1 --> S2 --> S3 --> S4
    end
    C4 -- "HTTP + traceparent" --> S1
```
...
- [Tracing notebook](https://github.com/ag2ai/ag2/blob/main/notebook/agentchat_tracing.ipynb) -- Interactive Jupyter notebook walkthrough
