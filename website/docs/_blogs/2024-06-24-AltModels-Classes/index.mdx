...
- **Utilise models specifically for chat, language, and coding; multimodal (image, audio, video) support is currently only available via Google Gemini.**
...
**Multi-modal support, such as for image/audio/video, is currently only available via the [Google Gemini](/docs/user-guide/models/google-gemini) client class.** This can be used to create a multimodal agent. Multimodal support for other providers (Mistral, Anthropic, Groq, Together.AI) is an area of active development, but not yet available.
