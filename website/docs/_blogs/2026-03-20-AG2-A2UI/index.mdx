...
```python
from autogen.agents.experimental import A2UIAction
...
## Transport: AG-UI and A2A

A2UIAgent produces A2UI JSON — a transport-agnostic UI specification. The same agent connects to frontend renderers through two transport options:

![A2UIAgent Architecture](img/a2ui-architecture.png)

### A2A Transport (JSON-RPC)

Wrap in `A2aAgentServer` for serving via the A2A protocol:
...
Use `AGUIStream` with the A2UI event interceptor to stream to AG-UI frontends:
...
The interceptor extracts A2UI JSON and emits `ACTIVITY_SNAPSHOT` events over SSE. Any AG-UI compatible frontend — CopilotKit, plain HTML, Angular — renders the A2UI surfaces.

Both transports deliver the same A2UI operations. The frontend renderer is transport-agnostic — it renders the same components regardless of how they arrived.

## Building a Marketing Preview Demo

Here's a complete example: an agent that generates email, LinkedIn, and X/Twitter preview cards for a product campaign.

```python title="a2a_agent.py"
from autogen.a2a import A2aAgentServer, CardSettings
...
agent = A2UIAgent(
    ...
    llm_config={"api_type": "google", "model": "gemini-3.1-flash-lite-preview", "api_key": "..."},
...
app = server.build()
# uvicorn a2a_agent:app --port 9000
```

Any A2UI-compatible client (web, Flutter, native) can connect to this agent and render the previews. The same agent serves all platforms.
...
The complete demo code is available in the AG2 repository under `autogen/agents/experimental/a2ui/examples/`.

## What's Next

A2UIAgent currently supports A2UI v0.9 with the basic catalog. Coming soon:

- **v0.10 support** — Enhanced catalog with more components and interaction patterns
- **More renderers** — Vue and native mobile renderers (Angular and Flutter demos already available)
- **Streaming A2UI** — Progressive UI updates as the LLM generates components

We'd love your feedback. Try A2UIAgent, build something with it, and let us know what you think on [GitHub](https://github.com/ag2ai/ag2) or [Discord](https://discord.gg/ag2).
