...
![A single agent node: an inner LLM-and-Tools loop wrapped by the harness. AssemblyPolicy builds the context window before each call; four middleware call sites (on_turn, on_llm_call, on_tool_execution, on_human_input) wrap each operation; every event lands on the append-only MemoryStream; TokenMonitor and LoopDetector observe the stream; and a persistent Knowledge store (working memory, conversation summaries, event log) underpins it all — injected into assembly at the start of a turn and rewritten by aggregation at the end.](img/harness.webp)
...
When a threshold is crossed the monitor emits an `ObserverAlert` onto the stream. React to it with a tiny `@observer` that filters for those events:
...
@observer
async def report_alerts(events, ctx):
    for event in events:
...
```python
import asyncio
from autogen.beta import Agent, KnowledgeConfig, Middleware, observer, tool
from autogen.beta.aggregate import AggregateTrigger, WorkingMemoryAggregate
from autogen.beta.compact import CompactTrigger, TailWindowCompact
from autogen.beta.config import OpenAIConfig
from autogen.beta.events import ToolCallEvent
...
async def main():
    agent = Agent(
        ...
        middleware=[Middleware(TelemetryMiddleware)],
        observers=[
...
- Docs: [The Agent Harness](/docs/beta/agent_harness) · [autogen.beta](/docs/beta/agents)
- Full examples: [autogen.beta reference notebooks](https://github.com/ag2ai/ag2/tree/main/notebook)

*Coming soon — a hands-on follow-up that builds a safe, long-running document-and-web research agent on this harness, end to end.*
