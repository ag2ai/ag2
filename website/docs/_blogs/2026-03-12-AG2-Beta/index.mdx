---
title: "AG2 Beta: A Better Foundation for Real-World Agents"
authors: [Lancetnik]
...
We found that some of these needs were awkward to address cleanly inside the original framework model. In many cases, shipping more production-suitable behavior meant adding complexity around the edges instead of improving the core abstraction itself. That is a big part of why we decided to create **AG2 Beta**: a new framework track built around the lessons we learned from the original AG2 and designed to better support modern, real-world agent systems.
...
In the previous implementation, an agent instance effectively carried the active conversation with it. That was convenient for small examples, but it became awkward in real applications:
...
- integrating agents into chat platforms, web apps, and background workers was harder than it should be

This is exactly the kind of gap that shows up when you move from demos to production.

In a real application, the agent definition should be reusable. The conversation should be isolated. The application should decide how sessions are created, persisted, resumed, and discarded.

## The Core Idea: `MemoryStream`

**AG2 Beta** introduces `MemoryStream` as the conversation boundary. If you want the lower-level details of how streams work as the event bus behind a conversation, see [Events Streaming](/docs/beta/advanced/stream).
...
That gives you a much better model for real systems:
...
The same `agent` instance serves both conversations. The memory lives in `stream_a` and `stream_b`, not in the agent itself. That is the key property that makes the API much easier to embed into real applications.
...
To demonstrate that, we built a Telegram-based integration. The structure is intentionally simple:
...
The first version below is intentionally minimal. It focuses on the session boundary and message flow, while already passing `user_id` into the conversation variables so the next iteration can use that identity inside tools.
...
Short-term conversation state is handled by `MemoryStream`, but real assistants often need more than that. They also need a lightweight way to recall what happened earlier in the day, even after the in-memory conversation is cleared.
...
The next snippet extends the previous bot instead of replacing it. It shows only the additional pieces needed for long-term memory: a tool that can read saved history, a `/clear` command that persists the current stream before dropping it, and a helper that writes the conversation transcript to disk.
...
- Telegram chats, web sessions, and background jobs already have their own identity model
...
That is why `MemoryStream` is such an important primitive in **AG2 Beta**. It moves conversation ownership to the layer that already manages users and sessions.
...
That model is already enough to power ambient assistants, Telegram bots, web copilots, and background workers in a clean way.

<Note>
If you are waiting for multi-agent workflows, be patient, but you do not need to wait to start experimenting. Beta agents are already compatible with existing AG2 chat topologies through `as_conversable()`, including group chats, sequential workflows, and handoffs. See [AG2 Compatibility](/docs/beta/ag2_compatibility) for the current integration path.

You can introduce new agents in Beta API and use them in existing AG2 chat topologies.
</Note>

## Try AG2 Beta

If you want to understand **AG2 Beta**, start with the single-agent mental model:
...
That is a small API shift, but it unlocks a much better foundation for real systems.

If your current agent application needs isolated conversations, concurrent users, or lightweight long-term memory, this is a good time to try **AG2 Beta**.
