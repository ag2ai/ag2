...
response = agent.run(message="Solve this complex problem...", max_turns=2).process()This pattern ensures:
...
response = agent.run(message=prompt, max_turns=2, user_input=True).process()This configuration enables:
...
result = research_assistant.run(
    ...
    max_turns=3,
).process()When `include_thoughts=True`, the agent's response will contain:
...
response = agent.run(message="Answer this straightforward question", max_turns=1).process()## Advanced Patterns
...
result = agent.run(message="Your query here", max_turns=2).process()### Pattern 3: A/B Testing Thinking Configurations
...
def compare_thinking_levels(query: str, api_key: str):
    ...
    for level in levels:
        ...
        result = agent.run(message=query, max_turns=2).process()
...
`thinking_level` is only supported by Gemini 3 models. If you see an error with Gemini 2.5, use `thinking_budget` instead:
hon
...
# Simple queries may not generate thoughts even with include_thoughts=True**5. Suboptimal Performance with thinking_budget on Gemini 3**
...
ython
   ...
   response = agent.run(message="Your query here", max_turns=2).process()
   5. **Review the documentation**: [Google Gemini Models](https://docs.ag2.ai/latest/docs/user-guide/models/google-gemini)

6. **Try the example notebook**: [Gemini Thinking Config Example](https://github.com/microsoft/autogen/blob/main/notebook/agentchat_gemini_thinking_config_example.ipynb)

## Additional Resources

- [AG2 Google Gemini Documentation](https://docs.ag2.ai/latest/docs/user-guide/models/google-gemini)
- [Gemini Thinking Config Example Notebook](https://github.com/microsoft/autogen/blob/main/notebook/agentchat_gemini_thinking_config_example.ipynb)
