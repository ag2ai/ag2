...
result = math_agent.run(
    message="Solve the equation: 2x + 5 = -25.",
    max_turns=5,
).process()### Example 2: Using Dict Schema
...
result = math_agent_dict.run(
    ...
    max_turns=5,
).process()### Example 3: Advanced Custom Formatting
...
print(json.dumps(tool, indent=2))Notice that:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                return None
    return None### Pattern 3: Schema Validation
...
if model not in supported_models:
    raise ValueError(f"Model {model} does not support Tool Use")**2. Schema Validation Errors**
...
try:
    ...
    print(f"Schema error: {e}")**3. Response Doesn't Match Schema**
...
Ensure all required fields are present and correctly formatted."""**4. AWS Credentials Issues**
...
try:
    ...
    print(f"Credentials error: {e}")**5. Tool Creation Fails**
...
print(json.dumps(tool, indent=2))## Benefits Summary
...
- [Structured Outputs Notebook](https://github.com/microsoft/autogen/blob/main/notebook/agentchat_bedrock_client_structured_output.ipynb)
