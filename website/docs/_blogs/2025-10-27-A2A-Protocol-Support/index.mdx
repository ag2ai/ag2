...
from autogen.a2a.server import A2aServer
...
Start the server with any ASGI server:
...
async def generate_and_review():
    ...
    result = await coder.a_initiate_chat(
        reviewer,
...
async def review_code(request: CodeReviewRequest):
    ...
    try:
        ...
        result = await submitter.a_initiate_chat(
            reviewer,
            message=message,
        )
...
result = await my_ag2_agent.a_initiate_chat(
    data_analyst,
...
```python
# This works
result = await agent.a_initiate_chat(remote_agent, message="...")

# This will fail
result = agent.initiate_chat(remote_agent, message="...")  # ❌
```
...
def sync_wrapper(agent, remote_agent, message):
    return asyncio.run(
        agent.a_initiate_chat(remote_agent, message=message)
    )
```
...
async def monitored_a2a_call(agent, remote_agent, message):
    start = time.time()
    try:
        result = await agent.a_initiate_chat(remote_agent, message=message)
...
server = A2aServer(
    ...
    description="Production code review service",
    # Additional config
    version="1.0.0",
    metadata={
        "supported_languages": ["python"],
        "max_code_size": 10000,
        "analysis_types": ["type_checking", "style"],
    },
)
```
