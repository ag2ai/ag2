...
is in place, import **`LiveAgent`**, **`SoundDevicePlayer`**, **`SoundDeviceRecorder`**, and the **`openai`** config helpers from **`autogen.beta.live`**. The **running** session is then just **three lines**: construct the agent, open the session and shared I/O on one **`async with`**, then block until you cancel.
...
Beyond the defaults, you can tune how the assistant **sounds** and **when it speaks**: OpenAI realtime **`AudioOutput`** accepts **`voice`** and **`speed`**, and **`InputConfig`** carries **VAD / turn-detection** options (for example semantic VAD with interruption). The [Providers (OpenAI)](/docs/beta/live/live_agent#providers){target="_blank"} tab lists voices and copy-pastable config.
...
- **OpenAI** — **`gpt-realtime-2`** with **`AudioOutput`** for **`voice`** and **`speed`**, or **`TextOutput`** for text-only replies; **`InputConfig`** controls **VAD and turn detection** (defaults lean toward semantic VAD with interruption). Voice list and snippets: [Providers (OpenAI)](/docs/beta/live/live_agent#providers){target="_blank"}.
...
## LiveAgent vs STT–TTS

AG2 Beta supports **two** voice patterns, documented in the [Voice & Realtime overview](/docs/beta/live/live){target="_blank"}:
...
| Typical latency | on the order of **1–3 s** per turn | **sub-500 ms** class in many setups |
| Turn detection | Your app decides when a “turn” ends | **Provider-driven** (e.g. semantic VAD, interruption) |
| Best when… | You need every **`Agent`** feature each turn (structured output, rich middleware, arbitrary model routing) | You need **phone-call-like** UX: continuous audio, interruption, minimal glue code |

They are complements. Use **STT–TTS** when the text agent is the source of truth for each step. Use **`LiveAgent`** when **latency and conversational flow** matter most and you are happy to drive the session through a **realtime** model such as **`gpt-realtime-2`**.
