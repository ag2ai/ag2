...
# TEMP Create handoffs and after works
customer_service.handoffs.add_llm_conditions(
[
    # One for each of the support agents
    # The available property for the billing agent should have an expression filter available condition = ${account_status} == "active"
]
)
customer_service.handoffs.set_after_work(RevertToUserTarget()) # TEMP

# For billing support need to be an existing customer
billing_support.handoffs.add_context_condition(
# where account_status is active as the available condition and condition as well ${account_status} == "active"
)

shipping_support.handoffs.set_after_work(AgentTarget(customer_service))
