...
```
┌─────────────┐      ┌─────────────┐      ┌──────────────────┐
│   AG2       │      │  Arcade.dev │      │  Gmail API       │
│   Agent     │─────▶│  Runtime    │─────▶│  Linear API      │
│             │      │             │      │  Slack API       │
│  (orchestr- │◀─────│  (auth,     │◀─────│  Google Drive    │
│   ation)    │      │   routing,  │      │  ... 8,000+      │
│             │      │   policy)   │      │                  │
└─────────────┘      └─────────────┘      └──────────────────┘
```
...
You'll need an [OpenAI API key](https://platform.openai.com/api-keys), an [Arcade API key](https://docs.arcade.dev/home/api-keys), and the email you registered with at [arcade.dev](https://arcade.dev).
...
We create thin wrappers that call `call_arcade_tool` with the Arcade tool name and inputs. The full example includes 11 tools (search, archive, trash, reply, drafts, and more). Here are three core ones:
...
The `AssistantAgent` decides which tools to call, the `UserProxyAgent` executes them. We register each function on both agents:
