...
The **Prompt Leakage Probing Framework** leverages key agentic design patterns from the [AG2 library](/website/docs/tutorial), providing a robust and modular structure for its functionality. Below are the patterns and components used, along with relevant links for further exploration:

1. **Group Chat Pattern**
   - The chat between agents in this project is modeled on the [Group Chat Pattern](/website/docs/tutorial/conversation-patterns#group-chat), where multiple agents collaboratively interact to perform tasks.
   - This structure allows for seamless coordination between agents like the prompt generator, classifier, and user proxy agent.
   - The chat has a [custom speaker selection](/notebook/agentchat_groupchat_customized) method implemented so that it guarantees the prompt->response->classification chat flow.

2. **ConversableAgents**
   - The system includes two [ConversableAgents](/website/docs/reference/agentchat/conversable_agent#conversableagent):
...
3. **UserProxyAgent**
   - A [UserProxyAgent](/website/docs/reference/agentchat/user_proxy_agent#userproxyagent) acts as the intermediary for executing external functions, such as:
...
Additionally, check out [AutoDefense](/website/blog/2024-03-11-AutoDefense), which aligns with future directions for this work. One proposed idea is to use the testing system to compare the performance of unprotected LLMs versus LLMs fortified with AutoDefense protection. The testing system can serve as an evaluator for different defense strategies, with the expectation that AutoDefense provides enhanced safety.
