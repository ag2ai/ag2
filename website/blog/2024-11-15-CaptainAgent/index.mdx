...
**TL;DR**
- We introduce CaptainAgent, an agent equipped with the capability to adaptively assemble a team of agents through retrieval-selection-generation process to handle complex tasks via the [`nested chat`](/website/docs/tutorial/conversation-patterns#nested-chats) conversation pattern in AG2.
...
The tool library we provide requires subscribing to specific APIs, please refer to the [docs](/website/docs/topics/captainagent/tool_library) for details. The following example does not necessarily require tool usage, so it's fine if you are subscribing to them.
...
# Further Reading
For a detailed description of how to configure the CaptainAgent, please refer to the [document](/website/docs/topics/captainagent/agent_library).
