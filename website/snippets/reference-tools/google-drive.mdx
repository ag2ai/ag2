...
3. Install AG2 with the `google-api` extra. Since our examples also use `openai`, install it as well:
    ...
    ```
    > **Note:** If you have been using `ag2` or `ag2`, all you need to do is upgrade it using:
    > ```bash
    ...
    > pip install -U ag2[openai,google-api]
    > ```
    > as `ag2` and `ag2` are aliases for the same PyPI package.
...
from ag2.tools.experimental.google import GoogleCredentialsLocalProvider, GoogleDriveToolkit
from ag2.tools.experimental.google.model import GoogleFileInfo
```
...
/Users/robert/projects/ag2/ag2/agentchat/conversable_agent.py:3091: UserWarning: Function 'download_file_from_drive' is being overridden.
...
Input arguments: {'page_size': 3}
/Users/robert/projects/ag2/ag2/agentchat/conversable_agent.py:3091: UserWarning: Function 'list_docs' is being overridden.
  warnings.warn(f"Function '{tool_sig['function']['name']}' is being overridden.", UserWarning)
user (to assistant):
