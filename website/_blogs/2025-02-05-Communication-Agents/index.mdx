...
Say hello to three new AG2 communication agents - **DiscordAgent**, **SlackAgent**, and **TelegramAgent**, here so that you can use an agentic workflow to send and retrieve messages from messaging platforms.
...
These are on be default, but you can turn these off by setting `has_writing_instructions` to `False` when creating the agent.
...
Get the latest bug report from Slack, then send it to Discord for a ticket to be raised, and then notify users of AG2 on Telegram. Each time a message is retrieved or prepared for send, use the tool_executor to execute that send/retrieve.
...
***** Suggested tool call (call_1yVp9XBZlzatDMlHou1zm5Fc): slack_retrieve *****
Arguments:
{"maximum_messages":1}
*******************************************************************************
...
Call ID: call_1yVp9XBZlzatDMlHou1zm5Fc
Input arguments: {'maximum_messages': 1}
tool_executor (to coordinator):

***** Response from calling tool (call_1yVp9XBZlzatDMlHou1zm5Fc) *****
...
***** Suggested tool call (call_PMnOJ9IxwZu13bTssoFY2YC3): discord_send *****
Arguments:
{"message":"🔧 **New Bug Report** 🔧\n\n*Description*: We have a bug with the **Swarm Orchestration**. It's crashing if two agents have the same name.\n\nPlease create a ticket for this issue!"}
*****************************************************************************
...
Call ID: call_PMnOJ9IxwZu13bTssoFY2YC3
Input arguments: {'message': "🔧 **New Bug Report** 🔧\n\n*Description*: We have a bug with the **Swarm Orchestration**. It's crashing if two agents have the same name.\n\nPlease create a ticket for this issue!"}
tool_executor (to coordinator):

***** Response from calling tool (call_PMnOJ9IxwZu13bTssoFY2YC3) *****
Message sent successfully (ID: 1336834678664986635):
🔧 **New Bug Report** 🔧
...
***** Suggested tool call (call_eYsSzczyGAE33Cbrmofe15sJ): telegram_send *****
Arguments:
{"message":"🔧 <b>New Bug Report</b> 🔧\n\n<i>Description</i>: We have a bug with the <b>Swarm Orchestration</b>. It's crashing if two agents have the same name.\n\nPlease assist in fixing this issue if you can!"}
******************************************************************************
...
Call ID: call_eYsSzczyGAE33Cbrmofe15sJ
Input arguments: {'message': "🔧 <b>New Bug Report</b> 🔧\n\n<i>Description</i>: We have a bug with the <b>Swarm Orchestration</b>. It's crashing if two agents have the same name.\n\nPlease assist in fixing this issue if you can!"}
tool_executor (to coordinator):

***** Response from calling tool (call_eYsSzczyGAE33Cbrmofe15sJ) *****
Message sent successfully (ID: 109):
🔧 <b>New Bug Report</b> 🔧
...
Discord message posted (notice it uses Markdown):
![Slack message](img/commsagents_discord_msg.png)

Telegram message posted (notice it uses HTML):
