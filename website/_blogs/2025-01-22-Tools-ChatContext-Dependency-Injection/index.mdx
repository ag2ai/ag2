...
In this post, we’ll build upon the concepts introduced in the [Tools with Dependency Injection](/blog/2025-01-07-Tools-Dependency-Injection/index) to demonstrate how to use [`ChatContext`](/docs/reference/tools/dependency_injection#chatcontext) for more advanced workflows.

By leveraging [`ChatContext`](/docs/reference/tools/dependency_injection#chatcontext), we can track the flow of conversations, ensuring proper function execution order. For example, before retrieving a user’s account balance, we’ll ensure the user has logged in first. This approach prevents unauthorized actions and enhances security.
...
The following [`BaseContext`](/docs/reference/tools/dependency_injection#basecontext) class and helper functions are adapted from
